...
### 1. `src/renderer/layout.rs`

- `Layout` 구조체에 `prev_para_was_empty_topandbottom_pic: Cell<bool>` 추가
- 신규 헬퍼 `paragraph_is_empty_topandbottom_pic(pi, paragraphs) -> bool`
- `layout_column_item::FullParagraph` 에서 caller 가 직전 paragraph 검사 후 set/reset
- `layout_shape_item` Picture 비-TAC + Para-relative 분기에서 빈 paragraph + TopAndBottom + caption 없음 시 `result_y += line_height + line_spacing`

### 2. `src/renderer/layout/paragraph_layout.rs`

- trailing-ls 제외 분기 (Task #479) 에 `!prev_was_empty_pic` 가드 추가
- 직전 paragraph 가 empty image-paragraph 였으면 trailing line_spacing 보존

### 3. `src/renderer/layout/integration_tests.rs`
...
- `cargo test --release` 18 스위트 모두 통과 (1125+ 테스트, 0 failures)
- 동일 패턴(빈 paragraph + TopAndBottom 그림) 보유 샘플 8개 시각 회귀 검증 — 모두 회귀 없음:
...
- 코드: `src/renderer/layout.rs`, `src/renderer/layout/paragraph_layout.rs`, `src/renderer/layout/integration_tests.rs`
...
- 단계별 변경 결정: Stage 2 보고서 (산식 (B) 채택, 후보 A 와 비교)
