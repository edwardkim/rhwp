...
10%에서 왼쪽 첫 페이지를 클릭한 뒤 50%로 확대하고 350px 순수 스크롤했다. 상태바의 viewport
페이지는 17쪽으로 바뀌지만 가로 눈금은 마지막 편집 focus인 왼쪽 열에 남는다. 오른쪽 페이지를
클릭하기 전에는 focus 눈금자가 스크롤만으로 이동하지 않는다.

![50%에서 순수 스크롤 뒤 상태바는 17쪽이고 눈금자는 마지막 편집 focus 열에 남은 rhwp-studio](../pr/assets/pr_6188_issue6149/focus_pinned_50_scroll.jpg)
