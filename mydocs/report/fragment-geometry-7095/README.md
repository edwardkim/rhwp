...
이 변경은 #7095의 **상자 기하와 분할 예산 부분 개선**이다. 칸 `valign` 및 기존 페이지별
내용 이동은 남아 있으며 이 보고서의 상자 좌표만으로 전체 이슈를 해결했다고 판정하지 않는다.
통합 PR 본문과 최종 squash 메시지는 `Refs #7095`를 사용하고 #7095는 OPEN으로 유지한다.
체리픽 원문에 포함된 종료 키워드를 최종 merge 메시지로 자동 복사하지 않는다.
[메인터너 검토](../../pr/archives/pr_7141_review.md)에서 새 검증과 잔여 범위를 확인한다.
아래 수치는 원 기여자의 초기 구현 기록이다.
