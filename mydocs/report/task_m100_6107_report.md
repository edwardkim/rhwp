...
가로·세로 이동과 한 쪽·두 쪽·맞쪽·여러 쪽 배치에서 상태 표시줄, 가로 눈금자와 세로 눈금자가
동일한 활성 페이지 snapshot을 사용하도록 통합했다. 캐럿·선택 페이지가 화면 안에 있으면 그 페이지를,
화면 밖이면 실제 2D 가시 페이지를 사용한다. 가로 PageUp/PageDown과 휠 좌우 변환은 Y축을 바꾸지 않고
X축 페이지 경계를 따라 이동한다.

실제 브라우저에서 한 화면의 왼쪽·오른쪽 또는 위·아래 페이지를 선택할 때 상태 표시줄과 두 눈금자가
같은 페이지로 함께 이동하는 것을 확인했다. Studio 전체 테스트, TypeScript 검사, 프로덕션 빌드와 기존
...
| 4 | 이 보고서 commit | 전체 회귀·빌드·실제 브라우저 검증 |
...
- `current-page-changed` 상태 표시줄과 Ruler는 같은 `ActivePageSnapshot`에서 파생된다.
...
- 가로 눈금자는 활성 페이지의 화면 X, 너비, 좌우 여백, 제본·맞쪽 정보를 사용한다.
- 세로 눈금자는 활성 페이지 한 쪽의 화면 Y, 높이와 위·아래 여백만 그린다.
- 여러 쪽 배치에서도 활성 페이지의 쪽 여백 핀을 표시한다.
- viewport fallback은 쪽 여백 핀을 유지하되 다른 페이지의 문단 들여쓰기 핀을 재사용하지 않는다.
...
tests 1153, pass 1152, fail 0, skipped 1
...
| 배치·입력 | 관측 | 판정 |
| --- | --- | --- |
| 자동/세로, 순수 스크롤 | `scrollTop 0 → 900`, 상태 `1 / 4 → 2 / 4`; 세로 눈금이 2쪽 로컬 좌표로 이동 | 통과 |
...
| 가로 이동, 다음 휠 | `scrollLeft 1638 → 1965`, 상태 `3 / 4 → 4 / 4`; 가로 눈금이 4쪽 위에 위치 | 통과 |
