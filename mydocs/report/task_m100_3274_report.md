...
- 기본 모드의 알 수 없는 옵션 무시(`_ => i += 1`)는 #3178 표준(exit 2)과 어긋나나,
  기존 스크립트 호환성 영향 조사가 필요해 본 PR 범위에서 제외 — 별도 이슈 후보.
- `batch ir-diff`(쌍 목록 배치 검증)는 stdin 입력 형식(쌍 구분) 설계가 필요해 수요 확인 후.
