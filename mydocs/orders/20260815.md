...
- 함께 제공된 `ROKG_R.ttf`는 원문 font table에 직접 등장하지 않는다. 정부상징 원본과 공통
  1,805자의 advance는 1,804자가 `0.001em` 이내지만 정규화 outline은 exact가 아니어서 비교
  자산으로만 두고 자동 alias로 취급하지 않는다. 두 파일은 `/home/edward/mygithub/ttfs/gov`의
  외부 검증 자산이며 저장소·PR에 포함하지 않는다.
