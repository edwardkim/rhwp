...
**머지 보류.** 메인터너 보정으로 PrEP 본문/표 겹침을 15건에서 1건으로 줄였고 67~70쪽의 국소 Visual Sweep은 통과했다. 추가 OLE 차트 보정 뒤 남성 차트가 있는 26쪽은 94.19%, 분리한 HWP/HWPX 샘플은 각각 90.67%/90.58%로 통과했다. 여성 차트가 있는 27쪽은 저장된 줄간격 복원 후 66.83%에서 91.85%로 개선됐다. 전체 140쪽 Native 검증은 양쪽 모두 140쪽을 출력했지만 9쪽이 실루엣 90% 미만이어서 `re_review_required`다. 40·51쪽의 표 이어받기 차이를 포함한 원인을 조사 중이고, 새 head의 전체 회귀·fresh WASM도 남았다. 현재 원 PR을 승인하거나 통합하지 않는다.
...
| Native Visual Sweep | 최신 영향 쪽 26/27/67/68/69/70/94/108의 실루엣이 각각 **94.19/91.85/99.99/96.50/99.94/100.00/99.57/99.84%**. 전체 140쪽은 rhwp/PDF 모두 140쪽. `re_review_required`: 1쪽 88.11%, 40쪽 60.17%, 51쪽 66.99%, 57쪽 88.08%, 61쪽 63.30%, 82쪽 75.40%, 90쪽 61.83%, 93쪽 75.61%, 105쪽 77.50%. 2·4·10·138쪽은 양쪽 모두 빈 페이지라 실루엣 지표가 없고 내용 픽셀 100% 일치 |
...
물리 69쪽의 표 보정과 94쪽 KoPub 대조군은 기존 이미지에 남겼다. OLE 분리 샘플의 백분율 축·막대 길이·색·레이블을 직접 확인했다. 원본 27쪽은 도형 줄 뒤에 저장된 720HU 줄간격을 복원해 다음 본문·그림 위치를 기준 PDF에 맞췄다. 전체 스윕의 40쪽은 1×1 셀 분할에서 원본 위·아래 안 여백 각 850HU와 rhwp 측정/컷 높이 차이 20.8px을 확인했다. 51쪽은 표의 두 행이 PDF보다 이른 쪽에서 이월된다. 두 차이는 직접 본 보류 사유다. 전체 140쪽의 모든 review·overlay는 Git 제외 `output/pr-review/planet6897-7406-20260925/visual/full-native-final/prep7406/`에 있다. 대표 이미지:
...
40·51쪽의 표 분할 차이와 다른 90% 미만 페이지를 순서대로 보정하고 영향 페이지를 다시 캡처한다. 사용자가 1쪽의 글꼴 차이는 무시하도록 지정했지만, 나머지 배치 차이와 자동 게이트 미달은 여전히 보류 사유다. 새 head의 fresh WASM Visual Sweep·nextest·lint를 마친 뒤 판정한다. 기준 PDF 140쪽과 rhwp 전체 출력 140쪽의 개수도 확인했으며, 선택 페이지만의 통과나 페이지 수 일치만으로 전체 시각 검증을 대신하지 않는다. 머지 시에는 실제 merge SHA와 CI URL, 같은 이미지의 merge SHA 고정 raw URL로 기여자에게 결과를 안내한다.
