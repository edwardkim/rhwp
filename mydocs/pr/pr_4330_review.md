...
검토에서 신규 CLI 통합 테스트가 nextest archive의 런타임 실행 파일 재매핑을 무시하는 차단점을
확인했고, 원 contributor history를 유지한 채 별도 보정 commit으로 해소했다.
...
| local code candidate | `3f60b28f685c66366d855f849e8d67e689b06387` |
...
| 명령 | 결과 |
| --- | --- |
| `CARGO_INCREMENTAL=0 cargo test --test schema_registry_contract` | 4 / 4 통과 |
| `rustfmt --check tests/schema_registry_contract.rs` | 통과 |
| `git diff --check origin/pr/4330..3f60b28f` | 통과 |

첫 cold build는 실행 시간 상한에서 종료됐고, 같은 checkout의 dependency cache를 유지한 재실행이
7분 21초 build 뒤 4개 테스트를 모두 통과했다. 전체 release-test·clippy는 focused 보정 범위를
검증하는 이번 단계에서 실행하지 않았다.
