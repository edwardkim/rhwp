# PR #6967 검토: HWPX 형광펜 범위의 왕복 보존

## 판정: 메인터너 보정 후 수용 가능

검토일: 2026-09-10. 검토자: jangster77.

메인터너 보정 적용 및 검증을 완료했다. run 내부의 형광펜 표식과 표 제어 슬롯 전후의 UTF-16 위치를 보존하고, HWP5 type-2 범위 태그와 COLORREF 변환을 구현했다. 텍스트 편집·분할·병합 시 위치 갱신도 반영했다. 집중 테스트 3개에서 반복 HWPX 변환, 표를 가로지르는 범위, 비백색 #123456의 0x02563412 변환을 검증했다. 실물 fixture의 형광펜 시작/끝 3쌍을 보존한 HWP를 한컴 2022에서 다시 열어 HWPX로 저장한 결과도 시작 3개·끝 3개·원본 색상 #FFFFFF 3개였다.

## 검토 기준과 출처

- 원 PR: https://github.com/edwardkim/rhwp/pull/6967
- 검토한 원 PR head: `e0565095b66e045a4ee3dc8215f71f2bb50e0571`.
...
| 원본 commit | 적용 commit |
| --- | --- |
| `e0565095b66e045a4ee3dc8215f71f2bb50e0571` | `15e674ad2750c8eeeb4a1d912595e7e968130b6f` |
...
- 실물 fixture: `samples/issue6956/3024739-exposure-algorithm-markpen.hwpx`
- 한컴 재개방 왕복 작업 ID: `84ca4e0a-505e-4f89-a555-538dd91190ee`
- 기존 HWP 왕복 시 표식 0개 및 반복 HWPX 변환 시 2쌍으로 줄던 보류 사유를 해소했다.
...
한컴 실물 왕복 검증은 hwp2024-mcp-convert의 engine 2020, 한컴 12.0.0.4605로 수행했다. 비백색 색상은 계약 테스트와 한컴 대조 범위 태그로 검증했다. 형광펜의 새로운 화면 페인팅 기능까지 구현·검증했다고 주장하지 않는다.
...
- 원 PR #6967에 체리픽 통합 수용 사실, 실제 통합 PR 번호와 merge SHA, 원 변경과 메인터너 보정의 차이를 기록한다. 원 PR을 직접 merge했다고 표현하지 않는다.
- Visual Sweep 및 검증 기록의 정본 direct link는 `https://github.com/edwardkim/rhwp/blob/<merge-commit-sha>/mydocs/pr/archives/pr_6967_review.md`로 고정한다.
- 실제 증적 범위: 시각적 highlight 페인팅을 새로 검증했다고 기록하지 않는다. 마크펜 집중 3개와 Hancom 2022 왕복 결과의 시작 3개, 끝 3개, #FFFFFF 3개 보존을 구조 증적으로 기록한다.
