...
# PR #6752 검토 - 호환 PDF 대체 폰트 지원 문자 조회 캐시

## 판정: 승인

현재 후보에서 조회 캐시 계약 9개와 전체 회귀가 통과했다. 통제된 입력의 PDF 바이트 및 ToUnicode 보존을 근거로 원 기여 범위에서 수용 가능하다.
...
- 원 PR: https://github.com/edwardkim/rhwp/pull/6752
- 기여자: `seo-rii`; 사전 리뷰어: `jangster77`.
- 검토한 원 PR head: `a3bc2e6afc96e61ff9fe09c97a43d61a6e7e44d3`.
- 이 PR의 마지막 체리픽 commit: `5dd06f0ffa3edd7064eefe99e3a7b56bd939825b`.
...
- 문자와 폰트 위치별 지원 여부만 보관한다. 문자 4,096개·논리 슬롯 262,144개 상한을 넘거나 fontdb가 바뀌면 부분 캐시를 버리고 기본 선택기로 복귀한다.
- 기존 face 순서·이미 사용한 face 제외·스타일/굵기/너비 호환 판정식을 유지한다. 선택된 폰트의 로드·임베딩 및 Skia 직접 PDF 경로는 바꾸지 않는다.
- 파일-backed 양성·음성 지원 결과 재사용, 상한 초과, fontdb 교체, 통제된 폰트 집합의 PDF 바이트 동등성 및 production ToUnicode 보존 테스트를 검토했다.
...
- [Analyze (rust)](https://github.com/edwardkim/rhwp/actions/runs/33941491744/job/101239726583): `SUCCESS`.
- [adapter inter-diff](https://github.com/edwardkim/rhwp/actions/runs/33941491788/job/101239719334): `SUCCESS`.
- [CI Impact Policy](https://github.com/edwardkim/rhwp/actions/runs/33942202065): `SUCCESS`.
...
화면을 바꾸지 않는 조회 비용 최적화다. stock/indexed PDF 바이트 동등성 및 검색 가능한 U+E104 보존 테스트가 이번 전체 실행에 포함됐다. 실제 문서 전체 export 성능이나 모든 폰트 조합을 새로 측정했다고 주장하지 않는다.

기여자의 과거 A/B 이미지와 이번 현재 후보 이미지는 공통 증적 문서에서 출처를 구분했다. 전체 픽셀 일치율만으로 승인하지 않았으며, 이번 PR의 명시된 계약을 기준으로 판정했다.

## 범위와 잔여 조건

원 PR의 44.2배 수치는 고립된 조회 20,000회 측정이지 전체 PDF 생성 속도가 아니다. 폰트 파일 내용이 export 중 변하지 않는 기존 전제를 유지하며, 내부 경고 로그까지 같아지는 것은 아니다.
...
- 원 PR: [#6752](https://github.com/edwardkim/rhwp/pull/6752). 직접 merge가 아니라 출처를 보존한 체리픽 통합으로 수용했음을 명시한다.
- 이슈 처리 범위: Closes 대상 [#6679](https://github.com/edwardkim/rhwp/issues/6679)에 캐시 회귀·PDF 바이트 동등성·ToUnicode 검증을 기록하고 실제 auto-close를 확인한다. [#536](https://github.com/edwardkim/rhwp/issues/536)는 관련 상위 범위이므로 이번 PR만으로 종료하지 않는다.
- 검증 내용과 한계: 조회 캐시 회귀 9건, PDF 바이트 동등성과 ToUnicode 보존 검증 통과 사실을 기록한다. 전체 export 시간은 측정하지 않았으므로 성능 단축 수치를 주장하지 않는다. 바이트·구조 계약 검증을 임의의 PNG 일치율로 바꾸지 않는다.
...
- 이 리뷰의 고정 링크: [개별 검토 기록](https://github.com/edwardkim/rhwp/blob/<merge-commit-sha>/mydocs/pr/archives/pr_6752_review.md).
