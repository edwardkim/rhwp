# PR #6960 검토: Studio 이미지 크롭 경로 일치

## 판정: 승인

검토일: 2026-09-10. 검토자: jangster77.

DOM과 CanvasKit의 이미지 크롭 좌표 계약을 검토했다. 갱신한 WASM으로 pic-crop-01.hwp 1쪽을 Chrome에서 Canvas2D, CanvasKit compat/default로 각각 출력했다. 직접 대조한 두 배너의 크롭 범위와 배치가 일치하며, compat/default PNG의 SHA-256도 동일하다. CanvasKit 런타임 준비 상태는 통과했고 숨은 Canvas2D 오버레이, 지원하지 않는 연산 및 렌더 오류는 없었다.

## 검토 기준과 출처

- 원 PR: https://github.com/edwardkim/rhwp/pull/6960
- 검토한 원 PR head: `dfc69fab6a8538d788cb51d363c6ce9baefdaec0`.
...
| 원본 commit | 적용 commit |
| --- | --- |
| `d0be5b384d8e53112101271636dc4c68bfe59990` | `298fab7e20dcc6dbb6a55fdf0b57b12c5e1ef189` |
| `8234bfc4893ee386cb2bb94466a62da1fa045788` | `d3e6f33ae30f3f8f541d5b854b150442518b3925` |
| `dfc69fab6a8538d788cb51d363c6ce9baefdaec0` | `1bf48e4cc1614885d81658d3b72208dfa20f2d32` |
...
![PR #6960 검토 증적 pr6960-canvas2d-p001.png](../assets/pr_6959_6983_planet6897_20260910/pr6960-canvas2d-p001.png)

![PR #6960 검토 증적 pr6960-canvaskit-p001.png](../assets/pr_6959_6983_planet6897_20260910/pr6960-canvaskit-p001.png)

이번 실행은 headless Chrome의 software surface 경로다. readinessGateRequired=false인 실행이므로 명시적 필수 readiness gate 또는 GPU 경로 통과로 확대 해석하지 않는다. 원 PR의 크롭 비교 예산 2%→3% 변경은 이 샘플에 한정하며 전체 렌더러의 정확도 보장으로 사용하지 않는다. 현재 수치로 확인하지 않은 diff ratio를 통과 근거로 기재하지 않는다.
...
- 원 PR #6960에 체리픽 통합 수용 사실, 실제 통합 PR 번호와 merge SHA, 원 변경과 메인터너 보정의 차이를 기록한다. 원 PR을 직접 merge했다고 표현하지 않는다.
- Visual Sweep 및 검증 기록의 정본 direct link는 `https://github.com/edwardkim/rhwp/blob/<merge-commit-sha>/mydocs/pr/archives/pr_6960_review.md`로 고정한다.
- 실제 증적 범위: 1쪽을 Chrome Canvas2D와 CanvasKit으로 각각 확인했다. 두 이미지가 로드되고 crop 배치가 일치했다. CanvasKit은 software surface였으며 GPU 경로나 명시적인 required readiness gate 검증으로 확대하지 않는다.
- 대표 이미지 안정 경로: [pr6960-canvas2d-p001.png](../assets/pr_6959_6983_planet6897_20260910/pr6960-canvas2d-p001.png). 코멘트에는 `![PR #6960 검증 pr6960-canvas2d-p001.png](https://raw.githubusercontent.com/edwardkim/rhwp/<merge-commit-sha>/mydocs/pr/assets/pr_6959_6983_planet6897_20260910/pr6960-canvas2d-p001.png)` 형식으로 이미지를 직접 표시한다.
- 대표 이미지 안정 경로: [pr6960-canvaskit-p001.png](../assets/pr_6959_6983_planet6897_20260910/pr6960-canvaskit-p001.png). 코멘트에는 `![PR #6960 검증 pr6960-canvaskit-p001.png](https://raw.githubusercontent.com/edwardkim/rhwp/<merge-commit-sha>/mydocs/pr/assets/pr_6959_6983_planet6897_20260910/pr6960-canvaskit-p001.png)` 형식으로 이미지를 직접 표시한다.
