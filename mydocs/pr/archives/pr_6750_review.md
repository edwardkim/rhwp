...
pr: 6750
author: emptinessform
---

# PR #6750 검토 - 저장 캐럿 조회 API의 셀 문맥 제한 안내

## 판정: 승인

저장 캐럿 API 주석과 실제 반환 계약이 일치한다. 현재 후보의 타입 검사와 전체 테스트가 통과했으며 별도 보정 없이 수용 가능하다.
...
- 원 PR: https://github.com/edwardkim/rhwp/pull/6750
- 기여자: `emptinessform`; 사전 리뷰어: `jangster77`.
- 검토한 원 PR head: `14e8a94891444750f2c6d61ac78d2922bd15a0e7`.
- 이 PR의 마지막 체리픽 commit: `abccd6eb7013a08ef7bc5560332c4f4184349587`.
...
- `get_caret_position_native`는 저장된 `doc_properties` 캐럿 스탬프를 읽고 `sectionIndex`, `paragraphIndex`, `charOffset`만 직렬화한다.
- `WasmBridge.getCaretPosition()`의 셀 문맥 부재와 현재 편집 커서 조회 경로를 안내하는 주석이 구현과 맞는다.
- 첫 기여자 `emptinessform`의 문서 기여다. 최초 기여라는 이유로 검증 기준을 낮추지 않았으며, 환영 댓글은 실제 merge 후 절차에 포함한다.
...
- [Analyze (rust)](https://github.com/edwardkim/rhwp/actions/runs/33936393542/job/101226873671): `SUCCESS`.
- [adapter inter-diff](https://github.com/edwardkim/rhwp/actions/runs/33936393523/job/101226868487): `SUCCESS`.
- [CI Impact Policy](https://github.com/edwardkim/rhwp/actions/runs/33937426079): `SUCCESS`.
...
주석만 변경하여 별도 시각 대조 대상이 아니다. 첫 기여자 처리와 환영 댓글은 실제 통합 merge 이후 post_merge.md에 따라 수행한다.

기여자의 과거 A/B 이미지와 이번 현재 후보 이미지는 공통 증적 문서에서 출처를 구분했다. 전체 픽셀 일치율만으로 승인하지 않았으며, 이번 PR의 명시된 계약을 기준으로 판정했다.

## 범위와 잔여 조건

반환 타입을 좁히거나 셀 문맥을 API에 새로 추가한 PR은 아니다. 통합본의 다른 PR 검증 게이트는 별도로 완료해야 한다.
...
- 원 PR: [#6750](https://github.com/edwardkim/rhwp/pull/6750). 직접 merge가 아니라 출처를 보존한 체리픽 통합으로 수용했음을 명시한다.
- 이슈 처리 범위: 본문에 closing issue가 없으므로 임의의 이슈를 만들거나 종료하지 않는다. 게시 직전 실제 closing references를 다시 확인한다.
- 검증 내용과 한계: 저장 캐럿 API 주석과 실제 반환 구현·타입의 일치, Studio 전체 및 TypeScript 검사 통과를 기록한다. 문서·타입 계약 변경이므로 별도 시각 증적은 해당 없다. 첫 기여자에게 'rhwp 첫 기여를 보내주셔서 감사합니다'라는 환영과 구체적인 기여 내용, contributor fork 보존 사실을 남긴다.
...
- 이 리뷰의 고정 링크: [개별 검토 기록](https://github.com/edwardkim/rhwp/blob/<merge-commit-sha>/mydocs/pr/archives/pr_6750_review.md).
