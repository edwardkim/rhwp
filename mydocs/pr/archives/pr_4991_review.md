...
| 항목 | 값 |
| --- | --- |
| 원 PR | [#4991](https://github.com/edwardkim/rhwp/pull/4991) |
| 작성자 | `kevin9327` |
| 원 기능 SHA | `099d4d01f60f` |
| 원 기능 제목 | feat(cli): edit insert-text — 문단 좌표에 새 텍스트 삽입 (#4990) |
| 통합 상태 | 기준선에 이미 포함됨. 중복 cherry-pick은 하지 않음 |
| 적용 순서 | `upstream/devel` 기준선 포함 확인 |
| 원격 base/head/규모/mergeability | GitHub API가 2026-08-17에 `502`/`504`로 응답해 이번 기록에는 미확인으로 남김. 통합 PR 생성과 merge 직전에 재확인 필요. |

## 범위와 통합 판단

원 기능 commit은 현재 기준선에 이미 도달해 있으므로, 누적 통합 branch에 같은 변경을 다시 적용하지 않았다.
