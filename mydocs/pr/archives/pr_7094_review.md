...
| 항목 | 확인값 |
| --- | --- |
| 원 PR | [#7094](https://github.com/edwardkim/rhwp/pull/7094) — 수정: 합성 쪽나눔을 HWP5 저장본에 쓰지 않는다 — 한글이 쪽을 불린다 (#4680) |
| 작성자 / reviewer | planet6897 / jangster77 사전 지정 |
| 원 base / 규모 | `devel`; 2 files, +129/-0 |
| source head | `3ba97a74cde1cc927c31b5843be2bcf91de2b07d` |
...
| 관련 이슈 | [#4680](https://github.com/edwardkim/rhwp/issues/4680); 부분 반영 범위만 판단, 이번 작업에서 종료하지 않음 |
...
| --- | --- | --- |
| `3ba97a74cde1cc927c31b5843be2bcf91de2b07d` | `69353a38423e186107ae230a1d9393e330432bc9` | 충돌 없음; -x·저자 유지 |

### 실제 변경 경로

- `src/serializer/body_text.rs` (+13/-0)
- `tests/cases/issue_4680_synthesized_page_break_not_saved.rs` (+116/-0)

## 조판 원칙과 원인 계층

렌더링이 합성한 쪽 경계는 저장된 명시적 Page break와 다르다. raw_break_flags가 있으면 원시 값을 우선하고, 합성 Page만 직렬화에서 제외한다.

주요 검토 위치: [src/serializer/body_text.rs](../../../src/serializer/body_text.rs).

| 공통 항목 | 판정 | 근거·제한 |
| --- | --- | --- |
| 구현 근거와 일반성 | 비해당 | 렌더링이 합성한 쪽 경계는 저장된 명시적 Page break와 다르다. raw_break_flags가 있으면 원시 값을 우선하고, 합성 Page만 직렬화에서 제외한다. |
...
HWP5 PARA_HEADER의 break byte를 실제 CFB BodyText에서 읽었다. 기존 german-legislative-system.hwp를 두 바이너리로 변환한 결과 Page(4) 레코드 172→33, None(0) 3530→3669로 139개만 이동했다. 명시적·원시 Page와 Column 보존은 원 PR 회귀 계약으로 확인했다.

이번 검토 범위에서 새 실행 회귀 또는 수용을 막는 코드 문제를 발견하지 않았다. 아래 잔여·미검증 범위는 승인 대상에 포함하지 않는다.

**잔여·미검증:** 한컴에서 재저장본 전체 274/264쪽을 다시 변환·대조한 결과가 아니다. #4680의 전체 HWP3 저장 충실도 해결이나 이슈 종료를 주장하지 않는다.
...
| --- | --- | --- |
| [tests/fixtures/issue_4680/german-legislative-system.hwp](../../../tests/fixtures/issue_4680/german-legislative-system.hwp) | 기존 실제 HWP3 직렬화 입력 | `543d67cdb4d84b876949cef4f4ec7435b99716d6fa7feebde57b024c88d40559` |
...
[적용·후속 단계](pr_7094_review_impl.md)
