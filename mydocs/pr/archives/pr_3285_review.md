...
- `5f495e60a`는 optional `textbox {control, paragraph}` 주소와 원문 char offset mapping을 추가한다.
...
- **권고: 보정 후 수용.** #3280 뒤 source head 일치 확인과 사용자 push 승인 뒤 두 보정을 source branch에만
  추가한다. code/test 보정이므로 full CI를 기다린다.
- `MERGEABLE`/`BEHIND`와 CI 성공은 작성 시점 참고값이며 merge 직전에 최신 상태를 재확인한다.
