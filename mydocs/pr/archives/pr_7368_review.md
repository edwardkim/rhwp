...
`cargo-nextest 0.9.145`로 전체 Rust 회귀 **10,204/10,204 통과**, Clippy 3종, Native Skia 3종, fresh WASM 대체 빌드·Visual Sweep을 완료했다. 현재 Mac 로컬 환경 지침에 따라 Docker를 사용하지 않았으며, WASM은 아래에 명시한 저장소 스크립트의 `--no-opt` 빌드 결과다. 원격 PR push·GitHub comment·GitHub 승인·merge와 원격 head CI는 아직 수행하지 않았다.
...
최종 승인 후 원 PR head에 보정 코드와 증적을 push한다면 PR 본문에 검증한 실제 head SHA와 위 두 기준 PDF·각 페이지 Native/WASM review·overlay 링크를 넣는다. 대표 HWP 4·5쪽과 HWPX 2·4쪽은 Native 및 WASM review·overlay를 `![HWP 5쪽 Native 비교](https://raw.githubusercontent.com/edwardkim/rhwp/<검증-head-sha>/mydocs/pr/assets/pr7368_review/hwp_review_005.png)`와 같은 Markdown 이미지로 직접 표시해 reviewer가 PR 본문에서 볼 수 있게 한다. 나머지 페이지는 위 표와 같은 직접 링크로 제공한다. URL이 실제 head에서 열리고 이미지가 보이는지 게시 뒤 확인한다.
