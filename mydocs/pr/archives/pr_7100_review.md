...
# PR #7100 — 표 셀 마지막 완전 빈 줄의 trailing 간격 검토
...
원 PR head `977f2caa5df9861b73eac35ddc38d8d707dedae9` → 현재 통합 이력의 cherry-pick `0a6224e8604e26837ea869ea5957c1565eb385f0`. 원 저자와 `-x` 출처를 보존했다.
...
| 항목 | 확인값 |
| --- | --- |
| 원 PR | [#7100](https://github.com/edwardkim/rhwp/pull/7100) — 수정(renderer): 칸 마지막 줄이 빈 줄이면 trailing 줄간격을 행 높이에서 뺀다 (#7097) |
| 작성자 / reviewer | planet6897 / jangster77 사전 지정 |
| 원 base / 규모 | `devel`; 2 files, +131/-2 |
| source head | `977f2caa5df9861b73eac35ddc38d8d707dedae9` |
...
| 관련 이슈 | [#7097](https://github.com/edwardkim/rhwp/issues/7097); 부분 반영 범위만 판단, 이번 작업에서 종료하지 않음 |
...
| --- | --- | --- |
| `977f2caa5df9861b73eac35ddc38d8d707dedae9` | `9a21d89f496074ebefd15dcb8867028e5052a78a` | 충돌 없음; -x·저자 유지 |

### 실제 변경 경로

- `src/renderer/height_measurer.rs` (+32/-2)
- `tests/cases/issue_7097_cell_last_empty_line_trailing.rs` (+99/-0)

## 조판 원칙과 원인 계층

셀의 마지막 줄에 문자와 control이 모두 없을 때만 trailing 줄간격을 제외한다. 공백 문자만 있는 줄·객체만 있는 줄·기존 TAC 반례의 규칙을 혼동하지 않는다. 두 height-measurer 경로에 같은 조건을 적용한다.

주요 검토 위치: [src/renderer/height_measurer.rs](../../../src/renderer/height_measurer.rs).

| 공통 항목 | 판정 | 근거·제한 |
| --- | --- | --- |
| 구현 근거와 일반성 | 충족 | 셀의 마지막 줄에 문자와 control이 모두 없을 때만 trailing 줄간격을 제외한다. 공백 문자만 있는 줄·객체만 있는 줄·기존 TAC 반례의 규칙을 혼동하지 않는다. 두 height-measurer 경로에 같은 조건을 적용한다. |
...
36382471_masked.hwpx 1쪽과 148759031 실제 문서 3쪽을 동일 입력의 한컴 PDF와 전후 비교했다. 전자는 마지막 행 높이 350.1→342.1px, 뒤 행 시작 890.1→882.1px에 대응하며 후자는 표 시작·뒤 표의 4px 이동이 한컴 쪽으로 개선됐다. 2쪽/5쪽을 각각 유지한다.

이번 검토 범위에서 새 실행 회귀 또는 수용을 막는 코드 문제를 발견하지 않았다. 아래 잔여·미검증 범위는 승인 대상에 포함하지 않는다.

**잔여·미검증:** 표지 제목의 글꼴 굵기·누락 이미지 placeholder 등 기존 차이는 남는다. 빈 줄에 글자가 없다는 이유만으로 통과시키지 않고 표 외곽·뒤 내용 위치를 확인했다.
...
- cell-empty-p001: [한컴 / 변경 전 / 통합 비교](../assets/pr7100_cell-empty-p001_3way.png), [한컴·통합 overlay](../assets/pr7100_cell-empty-p001_ovl.png)
- incheon-trade-p003: [한컴 / 변경 전 / 통합 비교](../assets/pr7100_incheon-trade-p003_3way.png), [한컴·통합 overlay](../assets/pr7100_incheon-trade-p003_ovl.png)
...
| --- | --- | --- |
| [samples/issue2470/36382471_masked.hwpx](../../../samples/issue2470/36382471_masked.hwpx) | 한컴 입력; cell-empty | `43572dad5e17395aa02d1b0000b736b8467278931086604776ef30393dd0f54b` |
| [pdf/issue2470/36382471_masked-hwpx-2020.pdf](../../../pdf/issue2470/36382471_masked-hwpx-2020.pdf) | 독립 한컴 PDF 2쪽; 직접 sweep 1쪽 | `c742f264ecab461c86f10198164a762a95dfe8aea71f6fb8c94b13f9041038e1` |
...
| [tests/fixtures/issue_7097/148759031-incheon-customs-trade.hwp](../../../tests/fixtures/issue_7097/148759031-incheon-customs-trade.hwp) | 한컴 입력; incheon-trade | `a6415cfc717d9aef9084b14742da3be0d325b2f8c1bcabf606c0cb76af873a05` |
| [pdf/pr-planet6897-20260914/incheon-trade-hwp-2020.pdf](../../../pdf/pr-planet6897-20260914/incheon-trade-hwp-2020.pdf) | 독립 한컴 PDF 5쪽; 직접 sweep 3쪽 | `ceedb1d01088b5e1425d5a0a5be5c1b8dc96f65951b647ed4fe4ca7787df7423` |
...
[적용·후속 단계](pr_7100_review_impl.md)
...
| --- | --- | --- | --- |
| cell-empty 1쪽 | 0 (없음) | 96.72847% | 36.60408% |
| incheon-trade 3쪽 | 0 (없음) | 91.67824% | 19.91149% |
...
대표 asset은 [비교 PNG](../assets/pr7100_cell-empty-p001_3way.png)이며, 실제 merge 뒤
`https://raw.githubusercontent.com/edwardkim/rhwp/<merge-commit-sha>/mydocs/pr/assets/pr7100_cell-empty-p001_3way.png`로 고정한다.
