...
| maintainer review 2 | 이 문서와 `pr_4410_review.md`의 trailing 갱신 commit |
...
5. 내부 링크, 문서 metadata, roadmap 집계, v1.1 JSON graph 불변식,
   `git diff --check`를 통과했다.
6. 후속 review 갱신을 design correction과 분리된 trailing commit으로 만든다.
7. 상위 작업이 push를 결정하면 직전 remote SHA, LFS 대상, dry-run을 다시 확인하고
   push 뒤 최신 문서-only aggregate를 기다린다.
8. merge는 작업지시자의 별도 승인 전에는 수행하지 않는다.

## rollback

문제가 생기면 review 2, `f42bf99f`, `2c0a70e8`, `67e7c3bc`를 역순 revert한다.
contributor commit은 amend, rebase 또는 force-push하지 않는다.
