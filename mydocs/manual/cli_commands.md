...
각주/미주 문단에 문단 서식을 적용한다. 코어 `apply_para_format_in_footnote_native`. `--section`/`--para`/`--ctrl`/`--props` 필수.

### `edit set-equation-properties <파일> --section N --para N --ctrl N --props <JSON> [-o <출력>] [--dry-run] [--verify] [--json]`
본문 수식 속성을 바꾼다. 코어 `set_equation_properties_native`. `--section`/`--para`/`--ctrl`/`--props` 필수 (예: `{"script":"x^2"}`).

### `edit set-form-value <파일> --section N --para N --ctrl N --value <JSON> [-o <출력>] [--dry-run] [--verify] [--json]`
본문 양식 개체 값을 설정한다. 코어 `set_form_value_native`. `--section`/`--para`/`--ctrl`/`--value` 필수 (예: `{"value":1}` 또는 `{"text":"입력값"}`).
...
표 셀 안 양식 개체 값을 설정한다. 코어 `set_form_value_in_cell_native`. 좌표와 `--value` 필수.
