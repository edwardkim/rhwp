...
| 표면 규모 | CLI 명령 **83개**(그중 `--json` 계약 **52개**, batch 축 **9개**) · MCP 도구 **83개**(무상태 67 + 세션 전용 16) |
...
**`edit` 하위 7개** — `fill-fields`·`replace-text`·`set-cell`·`insert-text`·
`insert-image`·`redact`·`sanitize`. 산출물은 **입력 형식을 보존**한다(HWPX → HWPX).
