...
- **사람의 업무**: 보고서 표의 수치를 하나씩 찾아 타이핑해 갱신
- **에이전트 시퀀스**:
  ```bash
  rhwp export-tables 양식.hwpx --json | jq '.tables[0].cells[:4]'                     # ① 표 격자 구조 확인
  rhwp edit set-cell 양식.hwpx --table 0 --row 2 --col 1 --text "1,234" -o 갱신본.hwp --json   # ② 셀 값 기록
  rhwp export-tables 갱신본.hwp --json | jq '.tables[0].cells[] | select(.row==2 and .col==1).text'  # ③ 재독 대조
  ```
- **기계 검증**: ③ 의 재독 텍스트가 ② 입력과 일치. `export-tables` 좌표계와 `edit set-cell` 좌표계가
  같으므로 **발견→편집→검증이 한 주소로 닫힌다**. 병합 셀(rowSpan/colSpan)도 격자 좌표로 식별된다.
  (실측: 정부 양식 `2025년 기부·답례품 실적 지자체 보고서_양식.hwpx` — 표 53개, 병합 포함)
- **절감**: 표 수치 수기 갱신 소멸. 실적 취합 데이터만 준비하면 양식에 자동 기입.

## 4. 아카이브에서 근거 찾기 — 검색 → 해당 페이지만 렌더
...
- **에이전트 시퀀스**:
  ...
  ```
- **기계 검증**: `pageCount` == 렌더 쪽수. 청크의 `page` 는 예제 4 의 검색 주소·렌더
...
| 7 | 보고서 표 수치 갱신(실적 취합) | `edit set-cell` (양식 실측 #3381) | ○ |
...
- **G1 `edit replace-text`** — #8 문구 치환 + #13 마스킹 해제. 공문 개정·연도 갱신이
  일상 업무라 대체 효과가 가장 넓다.
- ~~**G2 `edit set-cell`**~~ — ✓ 완료 (#3381). #7 실적·수치 갱신 해제.
