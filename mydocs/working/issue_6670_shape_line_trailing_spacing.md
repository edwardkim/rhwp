...
### 대안 B 는 측정으로 반증됐다 (2026-09-08)

`stored_frame_tail_overflow` 를 "본문 안"으로 제한하면(= 초과 0.1) 어떻게 되는지 쟀다.
`split_candidate_rows_height` 가 조각 높이고 `avail_for_rows` 가 본문 여유이므로 이 상한은
곧 초과 0 이다.

| | `issue_3931` 5건 |
|---|---|
| 현행(무제한 허용) | 4 통과 / **1 실패**(`pi23`) |
| B(본문 안 상한) | 1 통과 / **4 실패** — `hwpx_fragment_route`·`pi14_scan`·`page_count` 추가, `pi23` 은 **그대로 실패** |

목표 시험이 안 고쳐진 것이 결정적이다. 상한을 걸면 walk 가 4행을 쪼개는 대신 **통째로**
다음 쪽으로 미룬다 — `TABLE_SPLIT_RESULT cursor_row=0 end_row=4 consumed=101.1` (첫 조각에
4행 내용 0줄). 12줄 아니면 0줄이고 중간이 없어서, 첫 조각이 `HEAD_FRAGMENT_TEXT` 를
잃는다. 종전에 "11+5 가 된다"고 적었던 추정은 틀렸다 — 실제는 0+16 이다.

즉 이 무제한 허용은 장식이 아니라 조각을 만들어 내는 장치다. 남은 지렛대는 A(셀 높이)
쪽뿐이고, 그건 #5751·겹침 기준선과 충돌한다. 둘 다 제품 방향이라 #6842 에 넘겼다.
