...
이 감사에서는 제안 문서만 만들며 CLAUDE.md, 스킬, 제품 코드, 테스트를 수정하거나 실행하지 않는다.
GitHub comment/commit/push/PR/merge도 수행하지 않는다.
...
다음은 **미적용 제안**이다. 기존 프로젝트 개요, parser/IR 경계, Studio 규칙 링크는 유지한다.
긴 실행 명령을 다시 복제하지 않고 공통 import와 아래 완료 조건을 추가한다.
