...
bottom footnote lane으로 이어 그리고, 긴 각주 180도 p131에서 시작해 p132로 이어
그린다. 현재 rhwp는 두 각주 모두 전체를 marker page에 원자적으로 등록한다.

- p129: 각주 176의 `…일상생`까지 소유
- p130: 번호·separator를 반복하지 않고 `활이나 직업적 활동…`부터 이어진 뒤 각주
  177·178 배치
- p131: 각주 180의 `…별개의 세`까지 소유
- p132: 번호·separator를 반복하지 않고 `대로 호적 등으로…`부터 이어진 뒤 각주
  181 배치
