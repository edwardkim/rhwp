...
| `actionlint -ignore 'SC2012' .github/workflows/ci.yml` | 통과 |
| 변경 파일과 diff 수동 대조 | workflow 변경 1곳, Native Skia cache step만 변경 |

전체 `actionlint .github/workflows/ci.yml`은 기존 751행의 `ls`에 대한 `SC2012`로 실패한다.
변경 전 HEAD를 stdin으로 검사해도 같은 `SC2012`가 재현됐고, 해당 기존 진단만 제외하면 신규 오류는 없다.
이번 cache 정합성 PR에서 무관한 shell script를 함께 수정하지 않았다.
