...
- 현재 범위: **R1 온전한 셀·반복 제목행 구현 및 focused 검증 완료. 메인테이너 시각 판정 대기. R2 미완료.**
...
메인테이너 확인 대상은 **첫 셀의 빈 문단 간격, `직렬` 위치, 후속 쪽 반복 제목 셀**이다.
시각 통과를 아직 선언하지 않는다. Studio의 기존 WASM은 교체하지 않았으므로 이번 판정은 SVG를 사용한다.
