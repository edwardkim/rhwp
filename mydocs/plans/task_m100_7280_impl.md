...
- 상태: 2026-09-20 작업지시자 승인. [R1 완료](../working/task_m100_7280_stage2.md), [R2 구조 분리·책임 묶음 통합 검증 완료](../working/task_m100_7280_stage41.md), [R3 책임 묶음 구현·검증 완료](../working/task_m100_7280_stage53.md), [R4 각주·미주 책임 묶음 구현·검증 완료](../working/task_m100_7280_stage54.md). R5–R6 및 최종 제출 게이트는 남아 있다. 아래 절편별 잔여 목록은 각 단계 당시의 기록이다.
...
- 책임 묶음 구현 기록: [Stage53 R3 완료](../working/task_m100_7280_stage53.md), [Stage54 R4 완료](../working/task_m100_7280_stage54.md). R4 제품 `59256802c`에서 전체 10,096건 통과/0실패/기존 제외50, Native 353쪽·fresh Docker WASM 329쪽 데이터와 선택19쪽 출력 보존을 확인했다. 다음은 승인된 R5 전체이며 R5/R6 완료를 뜻하지 않는다.
...
R2–R5의 실제 호출자·쓰기 위치·기존 테스트 목록을 작업 기록에 고정하고 계산 → 조정 → 상태
적용 순으로 분리한다. Stage52까지의 작은 절편별 진행은 아래 §4.1의 책임 묶음 방식으로 전환한다.
...
### 4.1 R3–R5 책임 묶음 진행으로 전환

2026-09-22 작업지시자는 작은 절편 반복의 비효율을 지적하고 R3·R4·R5 단위로 범위를
확대하도록 지시했다. 이는 **진행·검증·보고 단위 확대**이며 조판 동작 변경 허가는 아니다.
R3–R5를 한꺼번에 뒤섞는 단일 변경으로 만들지 않고 아래 순서로 연속 진행한다.

| 진행 단위 | 이번부터의 완료 범위 | 다음 묶음으로 넘어가는 조건 |
| --- | --- | --- |
| R3 잔여 전체 | 행 스캔의 남은 판단/상태 적용, 표 본체 조정, continuation의 소유·재개·종료 경계까지 완성 | 표 계산·컷·예약·실제 배치·재개 계약 및 책임 묶음 검증 통과 |
| R4 전체 | 각주 예약과 미주 준비·측정·배치의 책임 분리, 표/본문과의 예약 의존 정리 | 번호·예약 순서·본문 예산·측정 무부수효과 보존 및 책임 묶음 검증 통과 |
| R5 전체 | 상태 소유·전이와 구역 조정 확정, 도메인 외부 직접 쓰기 정리, facade 유지 | 명시적인 상태 소유·전이 경계, 호출 순서·API 호환 및 책임 묶음 검증 통과 |

- 묶음 내부는 의미 있는 경계별로 검토·되돌리기 가능한 커밋을 만든다. 커밋마다 작업을
  종료하거나 새 승인·새 보고서를 요구하지 않는다. 묶음당 하나의 작업 기록에 누적한다.
- 작은 Query 하나씩이 아니라 연결된 계산·조정·상태 소유를 함께 정리한다. 계산을 옮기고
  모든 쓰기를 거대 부모에 남긴 상태는 해당 책임 묶음의 완료로 보지 않는다.
- 진행 중에는 호출/쓰기/단락 평가/소유권 대조와 필요한 컴파일·focused 검사를 수행한다.
  컷 소유·상태 수명·예약 순서처럼 위험한 경계는 중간 검증하되, 기계적인 이동마다 동일한
  누적 집중 suite·Clippy를 모두 반복하지 않는다. 검증한 SHA와 이후 변경은 구분한다.
- 묶음 끝에서 누적 코드에 fmt·정책·native Clippy·관련 집중 계약·전체 회귀와 §7.2의
  Native/fresh Docker WASM 대조를 수행한다. 앞 절편의 통과를 새 묶음의 결과로 대신하지 않는다.
- 기존 고정 baseline은 보존하고 각 묶음의 시작/종료 SHA를 기록한다. 검증 worktree는
  clean 상태와 증적 보존을 확인해 재사용하며, 내부 커밋마다 새 worktree를 만들지 않는다.
  기존 worktree·target·사용자 변경을 임의로 삭제하지 않는다.
- 정상적인 R3→R4→R5 진행에는 작은 절편별 재승인을 요구하지 않는다. 실제 회귀나
  의미 보존 불가능·IR/API/조판 정책 변경 필요가 발견되면 해당 경계를 멈추고 보고한다.
  원격 push·PR·댓글 및 제출 전 PR CI 전체 검증의 별도 승인 절차는 변경하지 않는다.

R6 변경 관리 정본·전체 통합·재계측과 최종 제출 게이트는 별도 완료 항목으로 유지한다.
...
1. 묶음 내부: 원본/변경 함수·호출자와 상태 쓰기를 대조하고, 연결된 변경 경계에 필요한
   컴파일·focused test를 수행한다. 기계적 이동마다 동일 누적 suite를 반복하지 않으며,
   위험한 소유권/상태 전이 경계의 검증은 묶음 종료까지 무조건 미루지 않는다.
2. 책임 묶음 완료(R3/R4/R5 각각): 정확한 종료 head에서 fmt·적용되는 테스트 정책·native
   Clippy·누적 집중 계약·전체 회귀를 실행한다. 동일 입력·설정의 baseline과 변경본에서 내용 소유권, 컷, 기하 및
...
2026-09-22 작업지시자 지시에 따라 Stage52 이후 **R3 잔여 전체 → R4 전체 → R5 전체**로
진행 단위를 확대했다. 이번 계획 변경 자체는 제품 수정이나 해당 묶음의 검증 완료를 뜻하지 않는다.
