...
- **상태**: R0 `qualified-red`, R1·R2 `qualified`; R3 승인 대기
...
`Q2-D5-R0` red 계약을 qualified-red로 고정했고 `Q2-D5-R1` unique-source prepared identity cache와
`Q2-D5-R2` opt-in font-by-key transport를 qualified로 마쳤다. 다음 승인은 제품 변경이 없는
`Q2-D5-R3` resource qualification만 연다. R3·N0·N1·N2는 각 결과 보고와
메인테이너 승인 뒤 다음 절편으로
넘어간다. R3가 resource-qualified가 아니면 N0/N1을 시작하지 않는다. commit·remote push·PR 생성·GitHub
