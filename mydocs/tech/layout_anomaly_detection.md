...
render_geom_diff  (두 렌더 사이 비교)         layout_anomaly  (렌더 한 장 안의 판정)
  ...
  ─ maxDisp, structureMismatch                  ─ overflow / off-canvas / overlap / empty_page
...
한다 — 이때도 `overflow`·`off-canvas`·`overlap`(확정 신호)만 종료 코드 3(`render-diff` 의
...
- `--json`: 판정 봉투 한 줄(`schemaVersion`, `pages[].overflow/offCanvas/overlap/emptyPage`, `offCanvasCount`, `hasSignal` 등).
- `--strict`: `hasSignal`(overflow·off-canvas·overlap 확정 신호)이 있으면 종료 코드 3. `empty_page` 는 제외.
