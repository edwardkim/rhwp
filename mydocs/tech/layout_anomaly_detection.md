...
- `--json`: 판정 봉투 한 줄(`schemaVersion`, `pages[].overflow/overlap/textOverlap/emptyPage`, `textOverlapCount`, `hasSignal` 등).
- `--strict`: `hasSignal`(overflow·overlap·text-overlap 확정 신호)이 있으면 종료 코드 3.
