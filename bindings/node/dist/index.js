...
function isKnownExitCode(code) {
  return code === EXIT_OK || code === EXIT_RUNTIME || code === EXIT_USAGE || code === EXIT_VERIFY || code === EXIT_VERIFY_PAGES;
}
var EXIT_OK, EXIT_RUNTIME, EXIT_USAGE, EXIT_VERIFY, EXIT_VERIFY_PAGES, RhwpError, BinaryNotFoundError, UsageError, RhwpRuntimeError, VerdictFailed, ProtocolError, SessionClosedError, EnvelopeKeyError, RhwpTimeoutError;
var init_errors = __esm({
  "src/errors.ts"() {
    ...
    SessionClosedError = class extends RhwpError {
    };
...
init_errors();
init_process();
var PlanResult = class extends Envelope {
...
export { BinaryNotFoundError, DEFAULT_SESSION_TIMEOUT_MS, DEFAULT_TIMEOUT_MS, Document, ENV_VAR, EXIT_OK, EXIT_RUNTIME, EXIT_USAGE, EXIT_VERIFY, EXIT_VERIFY_PAGES, Envelope, EnvelopeKeyError, FieldDef, IrSchema, Plan, PlanResult, ProtocolError, RhwpError, RhwpRuntimeError, RhwpTimeoutError, SUPPORTED_SCHEMA_VERSION, Session, SessionClosedError, TypeDef, UsageError, VERSION, VerdictFailed, VerifyReport, asEnvelope, batch, binaryName, buildFromIngest, bundledDir, camelKeys, capabilities, capabilitiesSchema, clearBinaryCache, clearPlanCapabilityCache, convert, createBrowserClient, createNodeClient, csvToTable, digest, explain, exportAgentManifest, exportCapabilitiesSchema, exportDoclang, exportHml, exportHwpx, exportIrSchema, exportMarkdown, exportOntology, exportPdf, exportPlanSchema, exportProvenanceMap, exportStructure, exportSvg, exportTables, exportText, extractData, extractPages, fields, fillFields, findBinary, info, inspect, irDiff, irSchema, isKnownExitCode, isSafeIdentifier, iterNdjson, openDocument, propertyKey, raiseForExit, renderDiff, replaceText, runJson, runNdjson, runPlan, runRaw, scan, search, setCell, snakeKeys, tableToCsv, thumbnail, toCamel, toSnake, verify };
