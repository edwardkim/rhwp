...
pub struct OoxmlSeries {
    ...
    /// [#6053] 계열 `<c:spPr><a:ln><a:noFill/>` — 선을 그리지 않는다는 명시.
    ///
    /// **주식형에서만 소비한다.** 코퍼스 주식형은 계열 전건이 이 표기를 달고 있어 선이 없고,
    /// 고저선·캔들만으로 그림이 선다. 표기가 없는 계열(한컴 편집기가 새로 더한 계열)은 기본
    /// 선 스타일이라 한컴이 선으로 그린다 — 그 차이를 담는 필드다.
    ///
