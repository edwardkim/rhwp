# CLAUDE.md — rhwp-macos (macOS Quick Look 확장)

이 파일은 `rhwp-macos/` 서브디렉토리에 한정된 에이전트 규칙이다.
프로젝트 전반 규칙(하이퍼-워터폴, 문서 생성 규칙, 타스크 진행 절차)은 루트 `CLAUDE.md`에서 상속된다.
...
- `ios/devel` 브랜치 기반 `macos/quicklook` 브랜치에서 작업
- **git worktree**로 별도 디렉토리(`/Users/melee/Documents/projects/rhwp-macos`)에 체크아웃
...
**상세 기술 결정**: 루트의 `rhwp_quicklook_macos_impl.md` 참조

---

## 필수 참조 문서

- 루트 `rhwp_quicklook_macos_impl.md` — 전체 구현 계획서 (포크 기반 + worktree)
- 루트 `CLAUDE.md` — 업스트림 기여 규칙 (하이퍼-워터폴)
...
본 프로젝트는 `postmelee/rhwp` 포크의 `macos/quicklook` 브랜치에서 작업한다. 업스트림(`edwardkim/rhwp`)에 기여할 때는 다음 규칙을 따른다.

### 분류 기준

| 변경 성격 | 작업 위치 | 상향 기여 |
|-----------|-----------|-----------|
| QL/Thumbnail Extension (`rhwp-macos/Sources/QLExtension/`, `ThumbnailExtension/`) | 본 브랜치만 | 해당 없음 |
| macOS 호스트 앱 UI (`rhwp-macos/Sources/HostApp/`) | 본 브랜치만 | 해당 없음 |
| 빌드 스크립트/프로젝트 설정 (`rhwp-macos/scripts/`, `project.yml`) | 본 브랜치만 | 해당 없음 |
| **CGTreeRenderer의 UIKit→CG 치환** (iOS에도 유용한 개선) | 본 브랜치 + **업스트림 PR** | **필수** |
| **FontFallback의 macOS 폰트 매핑 추가** | 본 브랜치 + **업스트림 PR** | **필수** |
| **Rust 코어 버그 수정 / 렌더링 개선** | **업스트림 PR 우선** → `git merge`로 수신 | **필수** |
| **FFI 시그니처 추가/변경 제안** | **업스트림 이슈 등록 + PR 제안** | **필수** |

### 상향 기여 절차

1. 본 브랜치(`macos/quicklook`)에서 먼저 구현/검증 — 동작하는 상태 확보
2. `edwardkim/rhwp`의 `ios/devel` 브랜치를 base로 PR 생성
3. PR 설명에 다음 포함:
   - macOS Quick Look 확장 개발 과정에서 발견된 배경
   - iOS에도 동일하게 유효한 이유
   - iOS 빌드 통과 검증 결과
4. PR 머지 후 `git merge upstream/ios/devel`로 동기화
...
/Users/melee/Documents/projects/rhwp-macos    ← macOS worktree (macos/quicklook) — 본 프로젝트
```
...
| `macos/quicklook` | `rhwp-macos` | macOS Quick Look 개발 (ios/devel 기반) |
| `local/task{num}` | `rhwp-macos`에서 분기 | 타스크별 작업 |
