---
name: rhwp-safe-edit
description: rhwp CLI로 HWP/HWPX 문서를 원본 훼손 없이 편집하는 안전 규약입니다. 편집은 1층 edit 하위명령(-o 산출 분리·--dry-run 선확인·--verify 자기검증)으로 수행하고, exit 3/4 판정을 예외가 아니라 봉투 데이터로 읽습니다. 트리거 — 사용자가 "누름틀 채워", "표 셀 값 바꿔/갱신", "문구 일괄 치환", "체크박스 켜", "도장/서명 붙여", "개인정보 마스킹", "안전하게 편집", "dry-run으로 먼저" 등을 요청할 때. 전체 명령 레퍼런스는 mydocs/manual/cli_commands.md.
---
...
이 스킬은 **새 편집 로직을 만들지 않는다.** 이미 devel 에 있는 `edit` 하위명령을
에이전트가 원본을 깨지 않고 부르게 배선한다.

권위 출처: [`mydocs/manual/cli_commands.md`](../../../mydocs/manual/cli_commands.md)
(§edit 각 절·§종료 코드 #2707)와
...
| 3 | `--verify` IR 차이 검출 | 고장이 아니라 **판정**. 산출물은 남는다 — `verify.diffCount` 를 읽는다 |
...
5. **exit 2 라고 stdout 을 버리지 마라.** 단건 명령 실패는 stdout 0바이트가 계약이지만,
   `csv-to-table` 은 **exit 2 에서도 `invalid[]` 봉투를 낸다**. 비어 있지 않으면 읽는다.
6. **`--data @파일` 은 UTF-8 이어야 한다.** CP949 저장본은
   `stream did not contain valid UTF-8` 로 exit 1.
7. **`batch fill` 은 행별 `notFound` 가 있어도 exit 0** 이다 — 데이터 품질 문제는 실행
   실패가 아니라 레코드 판정이다. 행 단위로 확인하지 않으면 조용히 덜 채운 산출물 N개가 나온다.
8. **`redact` 의 `findings[].raw` 는 원문 개인정보 그 자체**다. 봉투를 로그·이슈에 붙일
   거면 `--no-raw` 로 애초에 뺀다.
9. **`--verify` 통과 ≠ 완전 동일.** 자기 재파싱 게이트일 뿐이며, 무손실이 계약이면
   `ir-diff <원본> <산출> --json`(차이 시 exit 3)을 별도로 돌려 `categories` 를 읽는다.

## 하지 않는 것

- 새 `edit` 하위명령을 이 스킬에서 발명하지 않는다.
- gym 과제·채점·pack 을 이 스킬 경로에 끌어들이지 않는다.
- 온보딩·문서 트리아지 스킬 본문을 이 파동에서 고치지 않는다.
