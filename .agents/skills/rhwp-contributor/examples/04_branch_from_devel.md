...
권위: [references/branch-isolation.md](../references/branch-isolation.md).
픽스처: [fixtures/transcripts/fetch_devel.json](../fixtures/transcripts/fetch_devel.json).
...
1. `git fetch upstream devel` 이 첫 명령이다.
2. 브랜치 이름은 이슈가 정한 `feat/agent-contributor`.
3. base 는 항상 devel 이다.
