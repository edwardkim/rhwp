...
권위: [references/work-receipt-pointers.md](../references/work-receipt-pointers.md).
픽스처: [fixtures/envelopes/receipt_pointer.json](../fixtures/envelopes/receipt_pointer.json).
...
1. `rhwp replay --capsule` / `audit` / `lineage` 를 가리킨다.
2. `.agents/skills/rhwp-work-receipt/` 본문을 수정하지 않는다.
3. 새 `receipt` 명령을 발명하지 않는다.
