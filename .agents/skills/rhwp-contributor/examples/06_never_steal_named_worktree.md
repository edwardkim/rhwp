...
픽스처: [fixtures/layouts/forbidden-worktrees/registry.json](../fixtures/layouts/forbidden-worktrees/registry.json).
...
1. 대상 경로가 `git worktree list` 에 있으면 그 자리는 끝난 것이다.
2. 접미를 바꿔 새 경로를 만든다. `rhwp-desk*` 는 계열 전체가 금지.
3. 다른 작업의 브랜치를 reset 하지 않는다.
