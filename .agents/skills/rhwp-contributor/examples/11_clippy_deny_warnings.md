...
픽스처: [fixtures/envelopes/clippy_pass.json](../fixtures/envelopes/clippy_pass.json).
...
1. `cargo clippy -- -D warnings` 를 fmt 다음에 실행한다.
2. 같은 checkout 에서 cargo 를 동시에 돌리지 않는다.
3. 경고를 allow 로 숨기지 않는다.
