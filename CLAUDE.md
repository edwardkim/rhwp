...
- 증상이 발생한 계층과 원인을 추적한다. 잘못된 좌표를 clamp하거나 출력을 숨겨 겹침만 없애는
  처리를 원인 해결로 간주하지 않는다.
...
- 측정, 레이아웃, paint와 각 출력 backend가 같은 적용 조건과 좌표계를 쓰는지 확인한다.
...
- 렌더링 변경은 동일 입력의 기준 출력과 변경 전후 실제 출력을 직접 비교한다.
  페이지 내용과 비교 영역을 맞추고 겹침, 누락, 줄바꿈, 차트 종류와 축 등 주장한 의미를 확인한다.
  페이지 수, 텍스트 추출, 해시와 픽셀 점수만으로 시각 통과를 선언하지 않는다.
