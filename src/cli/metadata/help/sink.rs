...
mod tests {
    ...
    #[test]
    fn block_head_reads_only_two_space_headers() {
        assert_eq!(
            block_head("  fields <파일>"),
            Some(("fields", Some("<파일>")))
        );
        assert_eq!(
            block_head("  edit fill-fields <파일>"),
            Some(("edit", Some("fill-fields")))
        );
        assert_eq!(block_head("      --json  설명"), None);
        assert_eq!(block_head("명령:"), None);
        assert_eq!(block_head(""), None);
    }

    #[test]
    fn scope_keeps_only_the_wanted_block() {
        let lines = collect("info", None, || {
            emit("명령:".to_string());
            emit("  fields <파일>".to_string());
            emit("      필드 조사".to_string());
            emit(String::new());
            emit("  info <파일>".to_string());
            emit("      문서 정보".to_string());
            emit(String::new());
            emit("  search <파일>".to_string());
            emit("      검색".to_string());
        });
        assert_eq!(lines, vec!["  info <파일>", "      문서 정보"]);
    }

    #[test]
    fn a_section_title_ends_the_block() {
        let lines = collect("run", None, || {
            emit("  run <계획.json>".to_string());
            emit("      계획 실행".to_string());
            emit("프로토콜:".to_string());
            emit("      이건 다른 구역".to_string());
        });
        assert_eq!(lines, vec!["  run <계획.json>", "      계획 실행"]);
    }

    #[test]
    fn subcommand_scope_picks_one_block_out_of_a_group() {
        let lines = collect("edit", Some("redact"), || {
            emit("  edit fill-fields <파일>".to_string());
            emit("      누름틀".to_string());
            emit("  edit redact <파일>".to_string());
            emit("      마스킹".to_string());
            emit("  edit sanitize <파일>".to_string());
        });
        assert_eq!(lines, vec!["  edit redact <파일>", "      마스킹"]);
    }
}
