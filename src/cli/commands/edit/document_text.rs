...
pub(super) fn edit_insert_column_break(args: &[String]) -> i32 {
    ...
    // [#5019] 문단 시작(offset 0)이면 문단을 가르지 않고 그 문단에 단 나눔만 건다 —
    // 가르면 원 문단 모양(개요 수준 포함)을 물려받은 빈 문단이 앞에 남는다. 쪽 나눔
    // (#7218)이 세운 구조와 같다: CLI 는 선언적 속성 setter 로 보내고, 코어
    // `insert_column_break_native` 는 한/글 키 입력을 그대로 흉내내는 분할 계약으로 둔다.
    // 봉투는 문단 수 변화(`paragraphDelta`)와 단 나눔이 걸린 문단 좌표
    // (`columnBreakParagraph`)를 싣는다.
    let at_paragraph_start = offset == 0;
    let paragraph_delta: u32 = if at_paragraph_start { 0 } else { 1 };
