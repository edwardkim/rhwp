...
mod tests {
    ...
    fn missing_file_exits_1() {
        assert_eq!(
            run(&[PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .join("samples")
                .join("__no_such_q_cursor_model__.hwp")
                .to_string_lossy()
                .into_owned()]),
            EXIT_RUNTIME
        );
    ...
    #[test]
    fn form_sample_emits_cursor_model() {
        let path = sample();
        let model = load_model(&path).expect("form-01.hwp 를 열 수 있어야 한다");
        assert!(model.get("listCount").and_then(Value::as_u64).is_some());
        let root = model.get("root").expect("root 객체");
        assert!(root.get("paraCount").and_then(Value::as_u64).is_some());
        assert!(root.get("topPos").and_then(Value::as_u64).is_some());
        assert!(root.get("endPara").and_then(Value::as_u64).is_some());
        assert!(root.get("endPos").and_then(Value::as_u64).is_some());
        assert_eq!(model["listCount"], 2);
        assert_eq!(root["paraCount"], 13);
        assert_eq!(root["topPos"], 24);
        assert_eq!(root["endPara"], 12);
        assert_eq!(root["endPos"], 0);
        let lists = model
            .get("lists")
            .and_then(Value::as_array)
            .expect("lists 배열");
        assert!(lists.is_empty(), "form-01 은 본문 리스트만 있다");
        let env = envelope(&path, model.clone());
        assert_eq!(env["tool"], TOOL);
        assert_eq!(env["command"], COMMAND);
        assert_eq!(env["schemaVersion"], ENVELOPE_SCHEMA_VERSION);
        assert_eq!(env["version"], rhwp::version());
        assert_eq!(env["untrustedContent"], true);
        assert_eq!(env["untrustedFields"], json!(["source", "root", "lists"]));
        assert_eq!(env["source"], path);
        assert_eq!(env["listCount"], model["listCount"]);
        assert_eq!(env["lists"].as_array().map(|a| a.len()), Some(lists.len()));
        assert_eq!(run(&["--json".into(), path]), EXIT_OK);
    }

    #[test]
    fn receipt_sample_emits_cursor_model_when_present() {
        let path = receipt();
        if !path.is_file() {
            return;
        }
        let model = load_model(&path.to_string_lossy())
            .expect("issue-986-receipt.hwp 를 열 수 있어야 한다");
        let root = model.get("root").expect("root 객체");
        assert_eq!(model["listCount"], 325);
        assert_eq!(root["paraCount"], 3);
        assert_eq!(root["topPos"], 72);
        assert_eq!(root["endPara"], 2);
        assert_eq!(root["endPos"], 0);
        let lists = model
            .get("lists")
            .and_then(Value::as_array)
        ...
        assert_eq!(
            lists.len(),
            323,
            "listCount 325 = 첫 하위 리스트 2 + 항목 323"
        );
        let first = &lists[0];
        assert_eq!(first["listId"], 2);
        assert_eq!(first["isCell"], true);
        assert_eq!(first["hostListId"], 0);
    }

    #[test]
    fn source_never_calls_mutators() {
        let src = include_str!("rhwp-q-cursor-model.rs");
        ...
        assert!(code.contains("get_cursor_model_json"));
