//! 한 쪽의 마크다운을 조회한다.
//!
//! 기존 읽기 전용 [`DocumentCore::extract_page_markdown_native`] 만 부른다.
...
const TOOL: &str = "rhwp-q-markdown";
const COMMAND: &str = "markdown";
const UNTRUSTED_FIELDS: &[&str] = &["source", "markdown"];
const USAGE: &str = "rhwp-q-markdown <파일> --page <N> [--json]";
...
fn print_help() {
    write_stdout(
        &format!("{TOOL} v{} — 쪽 마크다운 조회", rhwp::version()),
        true,
    );
    ...
    write_stdout("문서를 고치지 않는다. 빈 마크다운도 성공이다.", true);
}
...
fn run(opts: &Options) -> i32 {
    ...
    let envelope = match markdown_envelope(&opts.path, opts.page, &core) {
        Ok(value) => value,
        Err(code) => return code,
    };
    if opts.json {
        ...
        let markdown = envelope["markdown"].as_str().unwrap_or("");
        write_stdout(markdown, true);
    }
    EXIT_OK
}
...
fn markdown_envelope(source: &str, page: u32, core: &DocumentCore) -> Result<Value, i32> {
    let markdown = match core.extract_page_markdown_native(page) {
        Ok(text) => text,
        Err(e) => {
            eprintln!("오류: 쪽 마크다운을 읽지 못했습니다 - {e}");
            return Err(EXIT_RUNTIME);
        }
    };
    Ok(json!({
...
mod tests {
    ...
    fn form01_page0_markdown_is_success() {
        ...
        assert_eq!(envelope["untrustedFields"], json!(["source", "markdown"]));
        assert_eq!(envelope["page"], 0);
        assert_eq!(envelope["source"], source);
        let markdown = envelope["markdown"].as_str().expect("markdown string");
        let char_count = envelope["charCount"].as_u64().expect("charCount");
    ...
    fn out_of_range_page_is_runtime() {
        ...
        let core = open_core(source).expect("open form-01.hwp");
        let err = markdown_envelope(source, 99, &core).unwrap_err();
        assert_eq!(err, EXIT_RUNTIME);
    }

    #[test]
    fn source_never_calls_mutators() {
        let src = include_str!("rhwp-q-markdown.rs");
        ...
        assert!(code.contains("extract_page_markdown_native"));
