...
pub(crate) fn parse_paragraph_list(
    ...
    loop {
        ...
        // Square wrap 그림 앵커 전용 문단: lh=th, ls=0 (보완7)
        // 그림 객체만 포함하고 텍스트가 없는 앵커 문단의 경우, 줄간격 배율이
        // lh/ls를 팽창시켜 wrap zone 시작 vpos가 틀어짐. 그림 높이는 wrap zone
        // end_vpos로 반영되므로 앵커 문단 자체는 최소 높이만 필요.
        {
            let is_square_pic_only = para.text.is_empty()
