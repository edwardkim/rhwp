...
#[test]
fn test_section_def_exact_ctrl_data_duplicate_is_serialized_once() {
    ...
    let sd = SectionDef {
        extra_child_records: vec![
            ...
            RawRecord {
                tag_id: tags::HWPTAG_CTRL_DATA,
                level: 2,
                data: additional_direct_ctrl_data.clone(),
            },
            RawRecord {
    ...
    let canonical_records: Vec<_> = records
        .iter()
        .filter(|record| {
    ...
    assert_eq!(
        ...
        1,
        "문단 슬롯과 SectionDef extra가 같은 CTRL_DATA를 소유해도 한 번만 출력해야 한다"
    );
    ...
    assert_eq!(
        records[section_def_pos + 1].tag_id,
        tags::HWPTAG_CTRL_DATA,
        "canonical CTRL_DATA는 SectionDef CTRL_HEADER 직후에 있어야 한다"
    );
    assert_eq!(records[section_def_pos + 1].data, canonical_ctrl_data);
    assert!(
        records.iter().any(|record| {
            record.tag_id == tags::HWPTAG_CTRL_DATA
                && record.level == 2
                && record.data == additional_direct_ctrl_data
        }),
        "payload가 다른 추가 직접 자식 CTRL_DATA는 보존돼야 한다"
    );
    assert!(
        records.iter().any(|record| {
            record.tag_id == tags::HWPTAG_CTRL_DATA
                && record.level == 3
                && record.data == nested_ctrl_data
        }),
        "중첩 control의 CTRL_DATA는 보존돼야 한다"
    );
}
...
/// 플래그)에 반영돼야 한다. 어긋나면 한글 2022 가 CTRL_HEADER 직후의 캡션
/// LIST_HEADER 유무를 오판해 문서 전체 개방을 거부한다(크롤 빈티지 40429 표
/// COM 이등분 실측, 양방향 반증). 종전 HWP3/HWPX 출처 표는 캡션을 방출하면서
/// 이 비트를 안 켜 개방 거부됐다.
#[test]
fn captioned_table_sets_common_attr_bit29() {
    ...
    let make_table = |caption: Option<Caption>| Table {
        row_count: 1,
        col_count: 1,
        row_sizes: vec![1],
        border_fill_id: 1,
    ...
    // 직렬화된 섹션 레코드에서 'tbl ' CTRL_HEADER 의 attr(payload 0..4)을 읽는다.
    ...
    let with = serialize_section(&Section {
        paragraphs: vec![Paragraph {
            char_count: 2,
            controls: vec![Control::Table(Box::new(make_table(Some(caption_fixture(
                "표 캡션",
            )))))],
            ..Default::default()
        }],
    ...
    let without = serialize_section(&Section {
        paragraphs: vec![Paragraph {
            char_count: 2,
            controls: vec![Control::Table(Box::new(make_table(None)))],
            ..Default::default()
        }],
        raw_stream: None,
        ..Default::default()
