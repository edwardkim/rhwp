...
#[cfg(test)]
mod transform_invalidation_tests {
    use crate::document_core::DocumentCore;
    use crate::model::control::Control;

    /// 한컴 편집기가 만든 34° 회전 그림 — 표 셀 안. `tests/issue_1279_picture_rotation_save.rs`
    /// 가 같은 샘플로 회전 행렬 계약(146B `raw_rendering`)을 고정한다.
    const SAMPLE: &str = "samples/ta-pic-001-r.hwp";
    const CELL_PATH: &str = r#"[{"controlIdx":2,"cellIdx":2,"cellParaIdx":0}]"#;

    fn cell_picture(core: &DocumentCore) -> &crate::model::image::Picture {
        let table = match &core.document.sections[0].paragraphs[0].controls[2] {
            Control::Table(table) => table,
            _ => panic!("샘플 좌표가 표가 아니다"),
        };
    ...
    fn loaded() -> (DocumentCore, Vec<u8>) {
        ...
        let core = DocumentCore::from_bytes(&bytes).expect("파싱");
    ...
    /// [#5890] 같은 값을 다시 지정하는 것만으로 한컴 원본 렌더링 행렬을 파괴하지 않는다.
    ///
    /// 종전 판정은 props JSON 텍스트에 변환 키가 등장하는지만 봤으므로, 값이 그대로여도
    /// 146B 행렬이 0B 로 날아갔다(직렬화기가 자체 생성 행렬로 대체).
    #[test]
    fn same_valued_transform_props_keep_raw_rendering() {
        ...
        assert_eq!(
            ...
            original,
            "값이 그대로면 원본 렌더링 행렬을 유지해야 한다 (#5890)"
        );
    }

    /// [#5890] 키 이름이 **문자열 값 안에** 등장하기만 해도 파괴되던 텍스트 스캔 판정 회귀.
    #[test]
    fn transform_key_quoted_inside_a_value_keeps_raw_rendering() {
        ...
        assert_eq!(
            cell_picture(&core).shape_attr.raw_rendering,
            original,
            "변환 키가 문자열 값 안에 있을 뿐이면 행렬을 건드리지 않는다 (#5890)"
        );
    }
    ...
    /// 실제 변환 변경은 종전대로 파생 상태를 무효화한다 — 직렬화기가 새 크기에 맞는
    /// 행렬을 다시 만들어야 하므로 원본 바이트를 비우는 계약은 유지된다.
    #[test]
