...
mod tests {
    ...
    #[test]
    fn html_paste_top_level_span_parses_inline_text_without_raw_tags() {
        let mut core = DocumentCore::new_empty();
        let paragraphs = core.parse_html_to_paragraphs(
            "<span style=\"color:#ff0000\"><strong>홍길동</strong><u> 부장</u></span>",
        );

        assert_eq!(paragraphs.len(), 1);
        assert_eq!(paragraphs[0].text, "홍길동 부장");
        assert!(
            !paragraphs[0].text.contains('<'),
            "최상위 span 내부 태그가 문서 문자로 남으면 안 된다"
        );

        let bold_shape_id = paragraphs[0]
            .char_shapes
            .iter()
            .find(|run| run.start_pos == 0)
            .expect("굵게 제목 시작 위치가 있어야 함")
            .char_shape_id;
        assert!(
            core.document.doc_info.char_shapes[bold_shape_id as usize].bold,
            "중첩 strong 서식이 제목 첫 글자에 적용돼야 함"
        );

        let underline_start = "홍길동".encode_utf16().count() as u32;
        let underline_shape_id = paragraphs[0]
            .char_shapes
            .iter()
            .find(|run| run.start_pos == underline_start)
            .expect("밑줄 직함 시작 위치가 있어야 함")
            .char_shape_id;
        assert!(
            matches!(
                core.document.doc_info.char_shapes[underline_shape_id as usize].underline_type,
    ...
    #[test]
    fn html_paste_list_items_become_bulleted_paragraphs() {
        let mut core = DocumentCore::new_empty();
        let paragraphs = core.parse_html_to_paragraphs(
            "<ul><li>첫 번째 <strong>항목</strong></li><li>둘째 항목</li></ul>",
        );
    ...
    #[test]
    fn html_paste_plain_long_line_is_split_before_layout() {
        let mut core = DocumentCore::new_empty();
        let text = "가".repeat(DocumentCore::FLUSH_LINE_CHAR_CAP * 2 + 1);
        let paragraphs = core.parse_html_to_paragraphs(&text);

        assert_eq!(paragraphs.len(), 3);
        assert_eq!(
            paragraphs[0].text.chars().count(),
            DocumentCore::FLUSH_LINE_CHAR_CAP
        );
        assert_eq!(
            paragraphs[1].text.chars().count(),
            DocumentCore::FLUSH_LINE_CHAR_CAP
        );
    ...
    #[test]
    fn html_paste_oversized_markup_falls_back_to_capped_paragraphs() {
        let mut core = DocumentCore::new_empty();
        let text = "가".repeat(DocumentCore::HTML_PASTE_MAX_BYTES + 1);
        let html = format!("<div>{text}</div>");
        let paragraphs = core.parse_html_to_paragraphs(&html);

        assert_eq!(paragraphs.len(), 101);
        assert!(paragraphs
            .iter()
            .all(|paragraph| paragraph.text.chars().count() <= DocumentCore::FLUSH_LINE_CHAR_CAP));
        assert_eq!(paragraphs.last().unwrap().text, "가");
    }
}
