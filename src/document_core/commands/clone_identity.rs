...
//!
//! This is not a clone/reference rewriter. Existing objects (including duplicate
//! identities) stay untouched. Walk the owned IR, not just top-level controls;
    ...
    while let Some(node) = pending.pop() {
        match node {
            ...
            Node::Control(control) => match control {
                ...
                Control::Footnote(note) => pending.push(Node::Paragraphs(&note.paragraphs)),
                Control::Endnote(note) => pending.push(Node::Paragraphs(&note.paragraphs)),
