...
pub struct PageBorderFill {
    ...
    /// [#5717] 쪽 테두리를 구역 **첫 쪽에만** 적용한다 (구역 정의 속성 bit 8).
    ///
    /// 레코드가 아니라 구역 속성에 있는 값이라, 파서가 구역을 읽을 때 이 레코드에 실어
    /// 렌더러로 전달한다.
    pub first_page_only_border: bool,
    /// [#5717] 쪽 배경(채우기)을 구역 **첫 쪽에만** 적용한다 (구역 정의 속성 bit 9).
    ///
    /// 한글 2022 실측: 이 비트가 선 문서(성북구 자원순환집행계획, `flags=0x00300200`)는
    /// 172쪽 중 **1쪽에만** 배경색이 칠해지고, 꺼진 문서(환경부 보도자료, `flags=0x0`)는
    /// 쪽 테두리가 3쪽 **모두**에 그려진다.
    pub first_page_only_fill: bool,
}
