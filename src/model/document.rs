...
pub struct Document {
    ...
    /// [Task #1001/#1007] HWP3 → HWP5 변환본 식별 플래그.
    /// HwpSummaryInformation 의 HWP3 시대 년도 + ParaShape/CharShape 비율로
    /// `parser/mod.rs` 가 결정. 변환본 specific paginator 분기 (cross-paragraph
    /// vpos reset 감지 등) 의 trigger 로 사용.
