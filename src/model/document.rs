...
impl Document {
    ...
    pub fn layout_profile(&self) -> crate::model::provenance::LayoutCompatibilityProfile {
        ...
        // 원본 HWP3→HWPX 는 hwp3-origin 마커만 있다. lineage 를 켜면
        // HWP3→HWP5 변환본 전용 계약(spacing_before *2 등)이 적용돼
        // 직파싱 HWP3(hwp3_layout=false, native=true)와 어긋나고 쪽이 +1 된다
        // (#3518). 그 산출물은 native HWP3 와 같은 레이아웃 계약을 쓴다.
        // HWP3→HWP5 변환본의 HWPX 는 hwp5-origin 마커가 함께 있어 제외한다.
