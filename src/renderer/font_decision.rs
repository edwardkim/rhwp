...
mod tests {
    ...
    #[test]
    fn stage1_candidate_identity_vectors_match() {
        let identity = json!({
            "sourceBoundaryId": "rust-metric.metric-alias",
            "candidateKind": "finite-mapping",
            "sourceFace": "HY각헤드라인M",
            "targetOrPolicy": "HYHeadLine-Medium",
            "conditions": {},
            "order": null,
        });
        assert_eq!(
            candidate_id(identity).unwrap(),
            "candidate.0b74264b87ed459ef5db"
        );
        let identity = json!({
        ...
        assert_eq!(
            candidate_id(identity).unwrap(),
            "candidate.28970f1984ed0e4ce06d"
        );
        let identity = json!({
            "sourceBoundaryId": "rust-metric.metric-table",
            "candidateKind": "metric-entry",
            "sourceFace": "HCR Batang",
            "targetOrPolicy": "metric-entry:0",
            "conditions": {
                "bold": false,
                "italic": false,
                "emSize": 1000,
                "latinRangeSymbol": "FONT_0_LATIN_RANGES",
                "hangulSymbol": "FONT_0_HANGUL",
            },
            "order": null,
        });
        assert_eq!(
            candidate_id(identity).unwrap(),
            "candidate.b461c7b55f3d0a77a68d"
        );
        let identity = json!({
            "sourceBoundaryId": "rust-style-resolution.legacy-latin",
            "candidateKind": "finite-mapping",
            "sourceFace": "HCI Poppy",
            "targetOrPolicy": "Palatino Linotype",
            "conditions": { "languageSlot": "1" },
            "order": null,
        });
        assert_eq!(
            candidate_id(identity).unwrap(),
            "candidate.fc3aabaf8bef3e4397ee"
        );
    }
}
