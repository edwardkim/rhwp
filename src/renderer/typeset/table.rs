...
use crate::renderer::typeset::signed_hwpunit;
...
///
/// 한/글은 값 0 표를 쪽 경계에서 나누지 않고 통째로 다음 쪽에 놓는다 — 저장소 정본
...
/// 두 좌표계가 아직 화해되지 않아(#7198 축) 표만 옮기면 글 위에 표가 그려진다 —
/// 1490000 149쪽 실측 겹침 134 -> 156. 오프셋 0 float 과 `treat_as_char` 표는 흐름이
/// 밴드를 실제로 소비하므로 해당 없다.
///
