...
impl HeightCursor {
    ...
    pub(crate) fn vpos_adjust(
        ...
        // **breakable 텍스트** 연속 문단은 overlap tail 이 아니라 정상 연속이다. 이때 page_base
        // drift 로 end_y 가 위로 보여도(end_y < y_offset) page_tail backtrack 으로
        // y_offset(=trailing 포함 정답)을 깎으면 같은 미주 연속 문단이 컬럼 하단에서 줄간격이
        // 좁아진다(3-11월 18쪽 pi=852→853). 텍스트는 컬럼/쪽으로 넘길 수 있어 frame-fit 이
        // 불필요하다. 반면 수식-only tail(#1274)은 atomic 이라 frame 안에 맞춰야 하므로
        // 제외하고 종전대로 backtrack 한다(equation_tail_fit 경로와 정합).
