...
Canvas2D FontFace 등록, 독립 SVG·PDF·인쇄, Rust layout, 임베디드 glyph resource 교체는
지원 범위가 아니다. 자동 backend 선택의 기존 적합성 검사를 우회하지 않으며, 호스트 목록만으로
CanvasKit eligibility를 보장하지 않는다. 이 기능의 직접 수용 검증은 명시적 CanvasKit 화면에서 한다.

공급자 교체·변경·해제는 이전 요청을 취소하고 연결 세대까지 검사하여 늦은 결과를 폐기한다.
호스트 목록과 바이트는 browser storage에 저장하지 않는다. renderer는 성공·실패·pending과
native Typeface를 문서 자원 경계에서 정리하고 새 페이지를 준비한다. 이 갱신은 문서 모델과
dirty/undo 상태를 변경하지 않는다. 내부 ID·cache key는 저장할 글꼴명이 아니다.
