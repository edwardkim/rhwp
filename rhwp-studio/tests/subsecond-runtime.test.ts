...
test('devtools websocket forwards patches, reconnects, and reloads once for a full build', async () => {
  ...
  const patchA = '{"HotReload":{"jump_table":{"lib":"/wasm/patch-a.wasm"}}}';
  const patchB = '{"HotReload":{"jump_table":{"lib":"/wasm/patch-b.wasm"}}}';
  sockets[0]?.onmessage?.({ data: patchA } as MessageEvent);
  sockets[0]?.onmessage?.({ data: patchB } as MessageEvent);
  sockets[0]?.onmessage?.({ data: new Uint8Array([1]) } as MessageEvent);
  assert.deepEqual(applied, [patchA], 'B waits until A commits');
  errorEvents.emit('error', { message: 'unrelated UI failure' });
  assert.deepEqual(applied, [patchA], 'an unrelated error cannot release B');
  errorEvents.emit('rhwp-subsecond-commit', {});
  assert.deepEqual(applied, [patchA, patchB]);
  errorEvents.emit('rhwp-subsecond-commit', {});
  ...
  sockets[1]?.onmessage?.({ data: '"FullReloadCommand"' } as MessageEvent);
  sockets[1]?.onmessage?.({ data: '"FullReloadCommand"' } as MessageEvent);
  sockets[1]?.onmessage?.({ data: patchB } as MessageEvent);
  assert.equal(reloads, 1, 'a stock dx full build reloads exactly once');
  assert.deepEqual(applied, [patchA, patchB]);
  ...
  const disconnectedSockets: FakeWebSocket[] = [];
  const late: string[] = [];
  const stop = connectSubsecondDevtools(
    { applySubsecondDevtoolsMessage: message => (late.push(message), 'patch-dispatched') },
    {
      location: { protocol: 'http:', host: 'localhost:7701' },
      createWebSocket: url => new FakeWebSocket(url, disconnectedSockets),
      setTimeout: () => 0,
      clearTimeout: () => {},
      reportSignal: () => {},
  ...
  const capturedHandler = disconnectedSockets[0]?.onmessage;
  stop?.();
  capturedHandler?.({ data: patchA } as MessageEvent);
  assert.deepEqual(late, [], 'a captured handler cannot dispatch after disconnect');
});

test('a patch timeout asks dx to rebuild before newer code reaches the old WASM', async () => {
  const { connectSubsecondDevtools } = await loadRuntime();
  const sockets: FakeWebSocket[] = [];
  const applied: string[] = [];
  const events = new FakeErrorEvents();
  let timeout: (() => void) | null = null;
  let reloads = 0;
  const disconnect = connectSubsecondDevtools(
    { applySubsecondDevtoolsMessage: message => (applied.push(message), 'patch-dispatched') },
    {
      location: { protocol: 'http:', host: 'localhost:7701' },
      createWebSocket: url => new FakeWebSocket(url, sockets),
      setTimeout: (callback, delay) => (delay === 10_000 ? (timeout = callback, 1) : 2),
      clearTimeout: () => {},
      reportSignal: () => {},
      errorEvents: events,
      reload: () => { reloads += 1; },
  ...
  const patch = (name: string) => `{"HotReload":{"patch":"${name}"}}`;
  sockets[0]?.onmessage?.({ data: patch('A') } as MessageEvent);
  sockets[0]?.onmessage?.({ data: patch('B') } as MessageEvent);
  timeout?.();
  assert.deepEqual(sockets[0]?.sent, ['{"RhwpFullRebuild":true}']);
  events.emit('rhwp-subsecond-commit', {});
  assert.deepEqual(applied, [patch('A')], 'B stays off the retiring WASM even if A commits late');
  sockets[0]?.onmessage?.({ data: '"FullReloadCommand"' } as MessageEvent);
...
test('a graceful dx shutdown still reconnects while the connector is active', async () => {
  const { connectSubsecondDevtools } = await loadRuntime();
  const sockets: FakeWebSocket[] = [];
  const scheduled: Array<() => void> = [];
  const disconnect = connectSubsecondDevtools(
    { applySubsecondDevtoolsMessage: () => 'not-hot-reload' },
    {
      location: { protocol: 'http:', host: 'localhost:7701' },
      createWebSocket: url => new FakeWebSocket(url, sockets),
      setTimeout: callback => (scheduled.push(callback), scheduled.length),
      clearTimeout: () => {},
      reportSignal: () => {},
      errorEvents: new FakeErrorEvents(),
    },
  );
  sockets[0]?.onclose?.({ code: 1001 } as CloseEvent);
  scheduled.shift()?.();
...
test('the dioxus-cli version is derived from the crate, so it cannot drift', async () => {
  const { dioxusCliSource } = await import('../../scripts/dioxus-cli-version.mjs');
  const repoRoot = new URL('../../', import.meta.url);
  const derived = dioxusCliSource(fileURLToPath(repoRoot));
  assert.equal(derived.kind, 'git');
  assert.match(derived.rev, /^[a-f0-9]{40}$/);
...
test('changing the reviewed dx patch prepares a fresh source checkout', async () => {
  ...
  assert.notEqual(
    dioxusCliSourceDir(source, '/repo', 'first patch'),
    dioxusCliSourceDir(source, '/repo', 'changed patch'),
  );
});

test('a failed dx checkout leaves the same patch ready for a clean retry', async () => {
  const { prepareDioxusCliSource } = await import('../../scripts/install-dioxus-cli.mjs');
  const upstream = mkdtempSync(path.join(tmpdir(), 'rhwp-dx-upstream-'));
  execFileSync('git', ['init', '--quiet', upstream]);
  const sourceFile = path.join(upstream, 'source');
  writeFileSync(sourceFile, 'old\n');
  writeFileSync(path.join(upstream, 'other'), 'stable\n');
  execFileSync('git', ['-C', upstream, 'add', 'source', 'other']);
  execFileSync('git', [
    '-C', upstream, '-c', 'user.name=test', '-c', 'user.email=test@example.test',
    'commit', '--quiet', '-m', 'base',
  ]);
  const rev = execFileSync('git', ['-C', upstream, 'rev-parse', 'HEAD'], { encoding: 'utf8' }).trim();
  writeFileSync(sourceFile, 'new\n');
  const patch = execFileSync('git', [
    '-C', upstream, 'diff', '--unified=0', '--no-ext-diff', '--no-textconv', '--no-color',
    '--abbrev=8', '--no-renames', '--no-indent-heuristic', '--inter-hunk-context=0',
    '--diff-algorithm=myers', '-O/dev/null', '--src-prefix=a/', '--dst-prefix=b/', 'HEAD',
  ], { encoding: 'utf8' });
  writeFileSync(sourceFile, 'old\n');

  const root = mkdtempSync(path.join(tmpdir(), 'rhwp-dx-installer-'));
  mkdirSync(path.join(root, 'scripts', 'patches'), { recursive: true });
  writeFileSync(path.join(root, 'scripts', 'patches', 'dioxus-cli-hotpatch-tip-dependents.patch'), patch);
  const source = { kind: 'git' as const, git: upstream, rev };
  assert.throws(() => prepareDioxusCliSource({ ...source, git: `${upstream}-missing` }, root));
  const prepared = prepareDioxusCliSource(source, root);
  assert.equal(readFileSync(path.join(prepared!, 'source'), 'utf8'), 'new\n');

  execFileSync('git', ['-C', prepared!, 'config', 'filter.hide.clean', "sed 's/tampered/new/'"]);
  writeFileSync(path.join(prepared!, '.git', 'info', 'attributes'), 'source filter=hide\n');
  writeFileSync(path.join(prepared!, 'source'), 'tampered\n');
  assert.throws(() => prepareDioxusCliSource(source, root));

  writeFileSync(path.join(prepared!, 'source'), 'new\n');
  writeFileSync(path.join(prepared!, '.git', 'info', 'attributes'), '');
  execFileSync('git', ['-C', prepared!, 'update-index', '--assume-unchanged', 'other']);
  assert.throws(() => prepareDioxusCliSource(source, root));
  execFileSync('git', ['-C', prepared!, 'update-index', '--no-assume-unchanged', 'other']);

  execFileSync('git', ['-C', prepared!, 'update-index', '--skip-worktree', 'other']);
  assert.throws(() => prepareDioxusCliSource(source, root));
  execFileSync('git', ['-C', prepared!, 'update-index', '--no-skip-worktree', 'other']);

  writeFileSync(path.join(prepared!, '.git', 'info', 'exclude'), 'hidden.rs\n');
  writeFileSync(path.join(prepared!, 'hidden.rs'), 'fn hidden() {}\n');
  assert.throws(() => prepareDioxusCliSource(source, root));
});
...
test('a drifted or loose pin stops the install instead of fetching the wrong dx', async () => {
  ...
  assert.throws(() => dioxusCliVersion(fakeRepository('1.2', '1.2.9')), /exact registry version/);
});
