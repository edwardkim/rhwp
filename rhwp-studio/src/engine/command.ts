...
export class ApplyCharFormatCommand implements EditCommand {
  ...
  execute(wasm: WasmBridge): DocumentPosition {
    ...
    for (let p = first; p <= last; p++) {
      ...
      const to = p === last ? end.charOffset : cell
        ? wasm.getCellParagraphLengthByPath(start.sectionIndex, start.parentParaIndex!, cellPathJsonForPara(start, p))
        : wasm.getParagraphLength(start.sectionIndex, p);
      if (to <= from) continue;
      this.entries.push({
        paraIndex: p, startOffset: from, endOffset: to,
        beforeRuns: this.readRuns(wasm, p, from, to),
      });
    }
        ...
        for (const entry of this.entries) {
          ...
          if (cell) {
            wasm.applyCharFormatInCellByPath(start.sectionIndex, start.parentParaIndex!,
              cellPathJsonForPara(start, p), from, to, JSON.stringify(this.props));
          } else {
            wasm.applyCharFormat(start.sectionIndex, p, from, to, JSON.stringify(this.props));
          }
          entry.afterRuns = this.readRuns(wasm, p, from, to);
        }
      } catch (error) {
        // 실패한 최초 실행은 history에 들어가지 않는다. 이미 적용한 문단도 되돌린다.
        try {
          for (const entry of attempted.reverse()) this.writeRuns(wasm, entry, entry.beforeRuns);
        } finally {
          this.entries = [];
        }
        throw error;
  ...
  private readRuns(wasm: WasmBridge, para: number, from: number, to: number): CharShapeRun[] {
    const { start } = this;
    return isCell(start)
      ? wasm.getCharShapeRunsInCellByPath(start.sectionIndex, start.parentParaIndex!, cellPathJsonForPara(start, para), from, to)
  ...
  private writeRuns(wasm: WasmBridge, entry: ParaFormatEntry, runs: CharShapeRun[]): void {
    ...
    if (isCell(start)) {
      wasm.setCharShapeRunsInCellByPath(start.sectionIndex, start.parentParaIndex!,
        cellPathJsonForPara(start, entry.paraIndex), entry.startOffset, entry.endOffset, runs);
    } else {
