...
#[test]
fn section_defs_bulk_old_reapply_converges_like_per_section() {
    // TS SetSectionPropsAllCommand.undo 의 실제 경로 — bulk 네이티브(재조판 1회,
    // gpt 3차 리뷰 성능 지적 해소)로 old 를 되돌려도 per-section 루프와 같은
    // 바이트 수렴을 만족해야 한다.
    let mut core = two_section_core();
    let before = core.export_hwp_native().expect("변경 전 export");
    let count = 2usize;
    ...
    // undo — bulk 일괄 재적용.
    let items: Vec<String> = olds
        .iter()
        .enumerate()
        .map(|(s, def)| format!("{{\"idx\":{s},\"def\":{def}}}"))
        .collect();
    let resp = core
        .apply_section_defs_bulk_native(&format!("[{}]", items.join(",")))
        .expect("bulk old 재적용");
    assert!(resp.contains("\"applied\":2"), "2 구역 적용 — {resp}");
    for cap in &caps {
        core.restore_section_raw_native(*cap).expect("raw 복원");
    }
    ...
    let after = core.export_hwp_native().expect("복원 후 export");
    let diff = first_diff(&before, &after);
    assert!(diff.is_none(), "bulk undo 수렴 — 첫 불일치 @{diff:?}");

    // 거부 계약 — 범위 밖 idx 는 부분 적용 없이 거부된다.
    core.set_section_def_all_native(&new_json).expect("재변경");
    let caps2: Vec<u32> = (0..count)
        .map(|s| core.capture_section_raw_native(s).expect("재캡처"))
        .collect();
    let bad = format!("[{{\"idx\":99,\"def\":{}}}]", olds[0]);
    assert!(
        core.apply_section_defs_bulk_native(&bad).is_err(),
        "범위 밖 구역은 거부돼야 한다"
    );
    // 거부는 캡처를 소비하지 않는다(dbb39210c 계약) — 같은 undo 재시도가 성공한다.
    core.apply_section_defs_bulk_native(&format!("[{}]", items.join(",")))
        .expect("거부 뒤 재시도");
    for cap in &caps2 {
        core.restore_section_raw_native(*cap).expect("raw 복원");
    }
