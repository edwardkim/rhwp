...
fn fixture_with_footer_text() -> PathBuf {
    ...
    doc.insert_text_in_header_footer_native(0, false, 2, 0, 0, "ABCDEF")
        .expect("기존 텍스트");
    let out = temp("fx");
...
#[test]
fn delete_text_from_existing_footer() {
    let inserted = fixture_with_footer_text();
    assert_eq!(footer_text(&inserted), "ABCDEF");
    let out = temp("out");
