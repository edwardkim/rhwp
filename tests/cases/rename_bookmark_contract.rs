...
#[test]
fn rename_bookmark_changes_name() {
    let src = sample();
    let inserted = temp("ins");
    add_bookmark(&src, "rhwp-ren-old", &inserted);
    let (sec, para, ctrl) = find_bookmark(&bookmarks_json(&inserted), "rhwp-ren-old");
    let out = temp("out");
    ...
    assert!(after["bookmarks"]
        ...
        .unwrap()
        .iter()
        .any(|b| b["name"] == "rhwp-ren-new"));
    assert!(after["bookmarks"]
        .as_array()
        .unwrap()
        .iter()
        .all(|b| b["name"] != "rhwp-ren-old"));
...
fn dry_run_json_has_fields_and_no_file() {
    ...
    add_bookmark(&src, "rhwp-ren-dry", &inserted);
    let (sec, para, ctrl) = find_bookmark(&bookmarks_json(&inserted), "rhwp-ren-dry");
    let out = temp("dry");
    let output = Command::new(rhwp_bin())
        .args([
            "edit",
    ...
    assert_eq!(v["name"], "rhwp-ren-dry-new");
