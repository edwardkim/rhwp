//! `rhwp-q-char-shape` CLI 계약. 실제 바이너리를 실행한다.
...
fn tool_bin() -> String {
    std::env::var("CARGO_BIN_EXE_rhwp-q-char-shape")
        .unwrap_or_else(|_| env!("CARGO_BIN_EXE_rhwp-q-char-shape").to_string())
}
...
fn run(args: &[&str]) -> Output {
    Command::new(tool_bin())
        .args(args)
        .output()
        .expect("rhwp-q-char-shape 실행 실패")
}
...
#[test]
fn json_envelope_on_form01_list0_para0_pos0() {
    let path = sample(SAMPLE);
    let source = path.to_str().expect("utf-8 path");
    let args = ["--json", "--list", "0", "--para", "0", "--pos", "0", source];
    let output = run(&args);
    assert_eq!(
        output.status.code(),
        Some(0),
    ...
    assert_eq!(v["command"], "char-shape", "{v}");
    assert_eq!(v["list"], 0, "{v}");
    assert_eq!(v["para"], 0, "{v}");
    assert_eq!(v["pos"], 0, "{v}");
    assert_eq!(v["source"], source, "{v}");
    let shape = v["charShape"].as_object().expect("charShape object");
    assert!(
        !shape.is_empty(),
...
fn unknown_flag_nope_is_usage() {
    let path = sample(SAMPLE);
    let source = path.to_str().expect("utf-8 path");
    let args = [source, "--list", "0", "--para", "0", "--pos", "0", "--nope"];
    let output = run(&args);
    assert_eq!(
        output.status.code(),
        Some(2),
...
fn missing_list_is_usage() {
    let path = sample(SAMPLE);
    let source = path.to_str().expect("utf-8 path");
    let args = [source, "--para", "0", "--pos", "0", "--json"];
    let output = run(&args);
    assert_eq!(
        output.status.code(),
        Some(2),
...
fn missing_cursor_is_empty_success() {
    ...
    let source = path.to_str().expect("utf-8 path");
    let args = [
        "--json", "--list", "0", "--para", "99", "--pos", "0", source,
    ];
    let output = run(&args);
    assert_eq!(
        output.status.code(),
...
fn source_never_calls_mutators() {
    let src = include_str!("../../src/bin/rhwp-q-char-shape.rs");
    ...
    assert!(src.contains("char_shape_set_json"));
