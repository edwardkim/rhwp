//! [#5376] M04-2: HWPX 편집 후 parse→serialize→reparse 왕복 property (IrDiff 0).
//!
...
//!
//! 비교는 기존 [`diff_documents`] 이다. HWP5 왕복은 M04-3.
//!
...
/// 텍스트가 있는 최소 HWPX. `hwpx_roundtrip_integration` Stage 1 이 IrDiff 0 을 이미 고정.
const FIXTURE_TEXT: &[u8] = include_bytes!("../../samples/hwpx/ref/ref_text.hwpx");
/// 2×3 빈 표. baseline 등급이라 무편집 왕복은 IrDiff 0.
const FIXTURE_TABLE: &[u8] = include_bytes!("../../samples/hwpx/ref/ref_table.hwpx");

/// CI 기본. `PROPTEST_CASES` 가 있으면 proptest 가 덮어쓴다.
const CI_CASES: u32 = 8;
const CI_MAX_SHRINK_ITERS: u32 = 32;

const FIND_NEEDLES: &[&str] = &["안녕", "Hello", "123"];
...
fn arb_replace_text() -> impl Strategy<Value = ExistingRunStep> {
    (
...
fn assert_edit_serialize_reparse(bytes: &[u8], steps: &[ExistingRunStep]) -> Result<(), String> {
    ...
    let out = serialize_hwpx(&edited).map_err(|e| format!("serialize: {e}"))?;
    let reparsed = parse_hwpx(&out).map_err(|e| format!("reparse: {e}"))?;
...
fn fixture_identity_ir_diff_is_zero() {
    for (label, bytes) in [("ref_text", FIXTURE_TEXT), ("ref_table", FIXTURE_TABLE)] {
...
fn inexpressible_steps_are_skipped_not_invented() {
    ...
    assert!(
        ...
        .expect("fill_fields"),
        "ref_text 는 누름틀이 없어 fill_fields 를 skip 해야 한다"
    );
    assert!(
        !apply_existing_step(
            &mut core,
        ...
        .expect("set_cell"),
        "ref_text 는 표가 없어 set_cell 을 skip 해야 한다"
    );
    assert!(
        !apply_existing_step(&mut core, &ExistingRunStep::SetCheckbox { occurrence: 0 },)
            .expect("set_checkbox"),
        "ref_text 는 □ 가 없어 set_checkbox 를 skip 해야 한다"
    );
}
...
proptest! {
    ...
    #[test]
    fn edited_ref_text_hwpx_roundtrips_with_zero_ir_diff(
    ...
    #[test]
    fn edited_ref_table_hwpx_roundtrips_with_zero_ir_diff(
