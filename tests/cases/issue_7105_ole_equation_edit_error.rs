...
//! 스크립트를 읽어 렌더하지만(#7105 증상 ①) 되쓰는 경로가 없다 — `raw_contents` 는 읽기
//! 전용이고, 편집분을 현행 문법으로 써 넣으면 한/글이 그 개체를 못 읽는다(한/글은 레거시
//! `\CMD … \TAB` 방언을 기대한다). 역방향 직렬화는 별도 범위다.
//!
//! 그래서 이 커밋이 닫는 것은 **진단 가능성**이다. 신고자가 겪은 세 갈래 중 이 저장소에서
//! 확정된 것은 편집 경로 부재 하나이고, 나머지는 이렇게 갈린다 — 신고 원본
