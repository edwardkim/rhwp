...
fn bin() -> String {
    std::env::var("CARGO_BIN_EXE_rhwp-q-control-layout")
        .unwrap_or_else(|_| env!("CARGO_BIN_EXE_rhwp-q-control-layout").to_string())
}
...
fn run(args: &[&str]) -> Output {
    Command::new(bin())
        .args(args)
        .output()
        .expect("rhwp-q-control-layout 실행 실패")
}
...
fn form01_json_envelope() {
    ...
    assert_eq!(v["tool"], "rhwp-q-control-layout");
    assert_eq!(v["command"], "control-layout");
    assert_eq!(
        v["schemaVersion"],
    ...
    assert_eq!(v["controlCount"], controls.len());
...
fn help_exits_ok() {
    ...
    assert!(text.contains("rhwp-q-control-layout"));
...
fn source_never_calls_mutators() {
    let src = include_str!(concat!(
        ...
        "/src/bin/rhwp-q-control-layout.rs"
    ));
    ...
    assert!(src.contains("get_page_control_layout_native"));
