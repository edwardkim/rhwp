...
fn issue_7095_terminal_fragment_box_counts_the_trailing_empty_line() {
    ...
    // 비끝 조각은 조각 끝의 **보이지 않는 잔여**를 칠하는 상자에서 뺀다. 이 칸은 저장 높이가
    // 282HU 라 쪽 고정 갈래(`row_heights[0] = pinned_height`)를 타지 못해 그 잔여가 그대로
    // 남아 있었다.
    //
    // | 쪽 | 끝 유닛 | 뺀 값 | 수정 전 | 수정 후 | 정본 괘선 |
    // | --- | --- | ---: | ---: | ---: | ---: |
    // | 16 | 글줄 | 줄 뒤 간격 1100HU = 14.67 | 990.10 | 975.40 | 977.07 |
    // | 17 | 글줄 | 같음 | 993.00 | 978.40 | 979.95 |
    // | 18 | 중첩 표 atom | 이송 여백 41.07 | 1006.20 | 969.40 | 970.99 |
    //
    // 아래 값은 render tree bbox 이고 실제 SVG 괘선(975.15 · 978.09 · 969.12)보다
    // 0.25~0.31px 크다. 정본과의 차 1.86~1.92px 는 끝 조각의 잔차 1.96px 와 같은 계통이다.
    for (page_index, top, want_bottom) in [
        (15u32, 201.30, 975.40),
        (16, 115.30, 978.40),
        (17, 115.30, 969.40),
    ] {
        ...
        assert!(
            ...
            "#7095: 비끝 조각({page_index}) 상자는 {top:.2} .. {want_bottom:.2} 여야 한다 — \
             수정 전 990.10 · 993.00 · 1006.20: y={:.2} bottom={:.2}",
            other.y,
...
/// [#7095] 저장 칸 높이가 조각 내용을 **담고 있으면** 줄 뒤 간격을 그대로 둔다.
///
/// 한/글은 분할 칸의 저장 높이를 조각 상자 높이의 합으로 적는다(#7347). 그 값이 조각 내용보다
/// 크면 한/글이 실제로 적은 높이이므로 잔여를 건드리지 않는다. 차감을 그 조각까지 넓히면
/// 아래 네 쪽이 정본에서 벌어진다 — 코퍼스 A/B 1,016문서가 낸 반례다.
///
/// | 문서 | 쪽 | 저장 칸 높이 | 조각 내용 | 정본 괘선 | 잘못 빼면 |
/// | --- | ---: | ---: | ---: | ---: | ---: |
...
///
/// 빼야 하는 쪽은 저장 높이가 내용보다 **작다** — 1382000 16·17쪽 `3.76`(칸 여백 141+141 뿐),
/// 정책연구 161쪽 `289.52 < 937.09`, 164쪽 `218.08 < 937.09`.
/// 중첩 표 이송 여백은 이 판정과 무관하다(18쪽 41.07px).
#[test]
fn issue_7095_stored_cell_height_that_covers_the_fragment_keeps_trailing_spacing() {
    let core = load_sample("samples/issue6782/1480000-201900042-chemical-labeling-standards.hwp");
    let frag = tables_of_para(&page_nodes(&core, 33), 118, 1, 1)
        .into_iter()
        .max_by(|a, b| a.height.total_cmp(&b.height))
        .expect("34쪽 첫 조각 표");
    let bottom = frag.y + frag.height;
    // 정본 괘선 1022.83. render tree bbox 는 그보다 조금 크다(clip 전 값).
    assert!(
        (bottom - 1023.6).abs() < 1.0,
        "#7095: 저장 높이가 조각을 담는 칸은 줄 뒤 간격을 유지해야 한다 — \
         정본 1022.83, 잘못 빼면 1021.47: {bottom:.2}"
    );
}
