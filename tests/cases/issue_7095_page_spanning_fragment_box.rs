...
fn issue_7095_terminal_fragment_box_counts_the_trailing_empty_line() {
    ...
    // **이어받은** 비끝 조각(17·18쪽)은 조각 끝의 보이지 않는 잔여를 칠하는 상자에서 뺀다.
    // 이 칸은 저장 높이가 282HU 라 쪽 고정 갈래(`row_heights[0] = pinned_height`)를 타지
    // 못해 그 잔여가 그대로 남아 있었다.
    //
    // | 쪽 | 조각 | 끝 유닛 | 뺀 값 | 수정 전 | 수정 후 | 정본 괘선 |
    // | --- | --- | --- | ---: | ---: | ---: | ---: |
    // | 16 | **첫** | 글줄 | — (해당 없음) | 990.10 | 990.10 | 977.07 |
    // | 17 | 이어짐 | 글줄 | 줄 뒤 간격 1100HU = 14.67 | 993.00 | 978.40 | 979.95 |
    // | 18 | 이어짐 | 중첩 표 atom | 이송 여백 41.07 | 1006.20 | 969.40 | 970.99 |
    //
    // **16쪽은 미해결이다.** 첫 조각이라 이 규칙의 대상이 아닌데 정본은 977.07 로 12.7px
    // 위에 있다. 코퍼스 A/B 가 첫 조각에서 한/글이 그 간격을 **그린다**는 반례 3건을 냈다
    // (정책연구용역사업 163쪽 1034.19 · chemical-labeling 34쪽 1022.83 ·
    // vietnam_labor 27쪽 1022.83 — 모두 `start_cut` 이 비었고 빼면 벌어진다).
    // 16쪽이 그 셋과 무엇이 다른지 밝혀지기 전까지 첫 조각은 건드리지 않는다.
    //
    // 아래 값은 render tree bbox 이고 실제 SVG 괘선(989.81 · 978.09 · 969.12)보다
    // 0.29~0.31px 크다.
    for (page_index, top, want_bottom) in [
        ...
        assert!(
            ...
            "#7095: 비끝 조각({page_index}) 상자는 {top:.2} .. {want_bottom:.2} 여야 한다 — \
             이어짐 조각 수정 전 993.00 · 1006.20: y={:.2} bottom={:.2}",
            other.y,
...
/// [#7095] **첫 조각**에서는 한/글이 조각 마지막 줄의 뒤 간격을 그린다.
///
/// 이어받은 조각의 잔여를 빼는 규칙(`single_cell_fragment_paint_trailing_trim_px`)을 첫
/// 조각까지 넓히면 아래 세 쪽이 정본에서 벌어진다 — 코퍼스 A/B 1,016문서가 낸 반례다.
///
/// | 문서 | 쪽 | 조각 | 정본 괘선 | 잘못 빼면 |
/// | --- | ---: | --- | ---: | ---: |
...
///
/// 같은 정책연구 문서의 161·164쪽은 **이어받은** 조각이고 정본이 1010.67 로 빼야 맞는다.
/// 첫 조각과 이어짐 조각이 정확히 갈리는 축이다.
#[test]
fn issue_7095_first_fragment_keeps_its_trailing_line_spacing() {
    ...
    assert!(
        (bottom - 1023.2).abs() < 1.0,
        "#7095: 첫 조각은 줄 뒤 간격을 유지해야 한다 — 정본 1022.83, 잘못 빼면 1021.47: \
         {bottom:.2}"
    );
}
