...
/// [#7095] 내용이 **쪽 상자를 넘는** 조각은 줄 뒤 간격을 그대로 둔다.
///
/// 줄 뒤 간격 차감(`single_cell_fragment_paint_trailing_trim_px`)을 그 조각까지 넓히면 아래
/// 세 쪽이 정본에서 벌어진다 — 코퍼스 A/B 1,016문서가 낸 반례다. 내용이 쪽 상자를 넘으면
/// 한/글도 상자를 쪽에 맞추므로, 그 자리에서 또 빼면 정본보다 위로 올라간다.
///
/// | 문서 | 쪽 | `row_h` vs `pinned` | 정본 괘선 | 잘못 빼면 |
/// | --- | ---: | --- | ---: | ---: |
/// | `정책연구용역사업 중간진도보고서…` | 163 | 83.76 > 83.71 | 1034.19 | 1020.96 |
/// | `1480000-201900042-chemical-labeling-standards` | 34 | 126.96 > 125.47 | 1022.83 | 1021.47 |
/// | `1490000-200800034_vietnam_labor_report` | 27 | 543.49 > 535.35 | 1022.83 | 1022.25 |
///
/// 빼야 하는 쪽은 모두 여유가 있다 — 1382000 16쪽 `788.55 < 830.65`, 17쪽 `877.51 < 890.67`,
/// 정책연구 161·164쪽 `937.09 < 947.31`. 경계는 0.05px 와 10.22px 사이라 넉넉하다.
/// 중첩 표 이송 여백은 이 상한과 무관하다(18쪽 41.07px ↔ 상한 4.26px).
#[test]
fn issue_7095_overflowing_fragment_keeps_its_trailing_line_spacing() {
    ...
    assert!(
        ...
        "#7095: 쪽 상자를 넘는 조각은 줄 뒤 간격을 유지해야 한다 — \
