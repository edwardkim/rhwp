...
fn issue_7095_terminal_fragment_box_counts_the_trailing_empty_line() {
    ...
    // 같은 표의 **비끝** 조각(16·17·18쪽)도 정본과 맞는다 — 조각 끝의 **보이지 않는 잔여**를
    // 칠하는 상자에서 뺀다. 이 칸은 저장 높이가 282HU 라 쪽 고정 갈래
    // (`row_heights[0] = pinned_height`)를 타지 못해, 그 잔여가 그대로 남아 있었다.
    //
    // | 쪽 | 조각 끝 유닛 | 뺀 값 | 수정 전 | 수정 후 | 정본 괘선 |
    // | --- | --- | ---: | ---: | ---: | ---: |
    // | 16 | 글줄 | 줄 뒤 간격 1100HU = 14.67 | 990.10 | 975.40 | 977.07 |
    // | 17 | 글줄 | 같음 | 993.00 | 978.40 | 979.95 |
    // | 18 | 중첩 표 atom | 상자 아래 이송 여백 41.07 | 1006.20 | 969.40 | 970.99 |
    //
    // 아래 값은 render tree bbox 이고 실제 SVG 괘선(975.15 · 978.09 · 969.12)보다
    // 0.25~0.31px 크다. 정본과의 차 1.87~1.92px 는 끝 조각의 잔차 1.96px 와 같은 계통이다.
