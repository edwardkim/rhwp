...
fn prep_1790387_page_count_pin() {
    ...
    assert_eq!(
        pages, 142,
        "issue2006 1790387 핀 142쪽 (한글 2022 정답지 146쪽, 잔여 -4). \
         141→144 는 #2279 TAC host 가산, 144→145 는 자간 글자폭-비례 landing, \
         145→142 는 #2373 host 가산 제거(모집단 +1 회귀 30건 소거 트레이드 — \
         본 문서는 ladder Δ-스텝(다음 sb 포함)을 host 가 소비해야 하는 소수 \
         형상으로, follower sb 트림 연쇄와 결합해 국소 판별 불가. #2373 잔존 \
         트랙). 실측 {}p — 130p 부근이면 tac 이미지 스택 미분할(#2006) 회귀, \
         142p 초과 개선 시 핀을 올릴 것(정답 146 방향).",
        pages
    );
}
