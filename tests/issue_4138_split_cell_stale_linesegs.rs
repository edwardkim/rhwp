...
fn split_cell_into_reflows_stale_segs_and_rebuilds_ladder() {
    ...
    // 한글 2024 COM 재실측(2026-08-25): 같은 1×2 분할 저장본 197쪽. 종전 rhwp 도
    // 197 이었지만 그것은 반각 낫표 ｢｣(47개)를 전각으로 재던 오분류(#6023)가
    // 잉여 줄바꿈을 만들어 세로 회계 편차를 상쇄한 우연이었다 — #6023 수정으로
    // reflow 줄 나눔이 한글과 문자 단위로 일치하게 되자(p5 일대 표본 전수 일치
    // 실측) 세로 축 잔존이 -1쪽으로 노출됐다(196, 한글 197). 이 핀은 분할-reflow
    // 계약의 회귀 검출이 목적이므로 현 정합 상태(196)를 고정하고, 세로 잔존은
    // #6046 이 소관한다. 이 값이 197 로 돌아오면 #6046 해소, 다른 값이면 회귀.
    let pages = saved_hwp_page_count_and_split_provenance(&doc);
    assert_eq!(
        pages, 196,
        "#4138 회귀: 저장 뒤 재파싱 쪽수 {pages} (한글 2024 실측 197, 현 정합 196 — #6046)"
    );
}
...
fn split_cells_in_range_reflows_stale_segs() {
    ...
    assert_eq!(
        ...
        196,
        "#4138 회귀(범위 분할): 저장 뒤 재파싱 쪽수 불일치 (#6023 정합 후 196 — #6046 참조)"
    );
}
