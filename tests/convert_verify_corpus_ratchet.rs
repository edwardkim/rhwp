...
const EXPECTED_FAILURES: &[(&str, &str)] = &[
    ...
    (
        "hwp3-sample10-hwpx.hwpx",
        "[#4677] 문단 6개(15487·18423·18927·21249·22284·23699)의 둘째 lineseg 가 \
         PARA_TEXT 밖을 가리킨다 — 예: 문단 15487 은 char_count=37 인데 text_start=40. \
         저장기가 그 줄을 빼므로 재파싱 IR 의 lineseg 개수가 2→1 로 줄어 여기 걸린다. \
         이 '손실' 이 곧 수정이다: 범위 밖 줄을 그대로 쓰면 한글 2022 는 본문을 통째로 \
         버린다. 실측(12.0.0.535) — 수정 전 변환본 0자·1쪽, 수정 후 1,473,322자·763쪽으로 \
         원본과 일치. 값을 바로잡아 등재를 지우려면 lineseg 를 어긋나게 만든 상류(HWP3→HWPX \
         경로의 textpos 기준)를 고쳐야 한다.",
    ),
];
