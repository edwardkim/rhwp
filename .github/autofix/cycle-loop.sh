...
# 반박 심사는 그대로 두고, 재시도 횟수로 달성한다. 오탐 1건이 봇 전체를 정지시킨다.
#
...
# 출력(GITHUB_OUTPUT): status=FOUND|NO-FINDING, slug, title, attempts
set -uo pipefail
...
aider_until_result() { # aider_until_result <logfile> <msgfile> [--read f ...]
  ...
  for TURN in 2 3; do
    ...
    run_aider "$LOG" "$@" --restore-chat-history \
      --message "아직 루트에 AUTOFIX_RESULT.md 가 없다. 지시서의 남은 단계(red 회귀 테스트 작성 → src/ 최소 수정 → AUTOFIX_RESULT.md 작성)를 지금 끝내라. 포기하는 경우에도 status: NO-FINDING 으로 AUTOFIX_RESULT.md 는 반드시 남겨라."
  done
}
...
for (( A=1; A<=MAX_ATTEMPTS; A++ )); do
  ...
  # ── 2) 확정 수정 또는 AI 탐색 (읽을 것은 전부 컨텍스트로 직접 준다)
  READS=()
  ...
  if [ -n "$KIND" ]; then
    ...
    aider_until_result ../fix.log "$TMP/msg.md" --read FINDING.md "${READS[@]}"
  else
    ...
    aider_until_result ../hunt.log "$TMP/msg.md" "${READS[@]}"
  fi
...
echo "모든 시도 소진 — 제출 기준을 넘는 발견이 확정되지 않았다. 기준은 낮추지 않는다."
